"""IR 기업 현황에 `contract_received` 칸 (화면 이름 `계약서 수신 여부`)

머리글은 뒤에 한 번 바뀌었다. **이름만 바뀌고 이 칸과 값은 그대로**라 이주할
것이 없다 — 그래서 여기 적는 이름은 저장 자리(`contract_received`)다. 화면
이름이 또 바뀌어도 이 글이 낡지 않는다.
