...
TEAM_TEMPLATES = [
    ...
    # IR 자료를 달라는 답이 오면 자료를 먼저 보내고 이 문구를 뒤에 보낸다.
    # {기업목록} 은 "[기업1] 샘플애그" 처럼 **지난 회차에서의 번호**로 채워진다 —
    # 투자사는 그 번호로 기억하고 있어서 번호가 없으면 어느 기업인지 못 찾는다.
    # 인사는 인사말이 맡는다 — 여기에 또 넣으면 "홍길동 대표님 안녕하세요" 가
    # 두 번 나간다(0025 가 고친 것). 링크는 붙이지 않는다 — 자료는 사람이
    # PC 카톡에서 파일로 첨부한다(0053).
    ("ir_delivery", "{기업목록} IR deck 먼저 전달드리겠습니다."),
    # 아래는 카톡방을 **연결하기 전에** 쓰는 문구다(전화·문자). 발송 화면이 아니라
    # 사람이 보고 읽는 용도라 여기 담아 두고 문구 관리에서 고친다.
    # 저장소 기본값은 가상 이름이다 — 실제 문구는 각자 채운다.
    ("connect_call", "안녕하세요 심사역님.\n스타트업 투자플랫폼 운영기업 ○○○ ASSET 입니다.\n기업 Deal 소개 관련하여 카톡으로 딜 공유 드리려고 먼저 전화 연락 드렸습니다."),
    ("connect_sms", "안녕하세요 심사역님.\n스타트업 투자플랫폼 운영기업 ○○○ ASSET 입니다.\n기업 Deal 소개 관련하여 카톡으로 딜 공유 드리려고 연락 드렸습니다."),
    ("connect_reinvite", "안녕하세요 심사역님.\n스타트업 투자플랫폼 운영기업 ○○○ ASSET 입니다.\n기업 Deal 소개 관련하여 다시 연락 드렸습니다."),
    # 기업 리마인드 — **제목과 머리말**이다. 그 아래 요약 숫자와 요청 투자사
    # 목록은 코드가 붙이고, 목록 아래 맺음말은 바로 다음 문구가 맡는다
    # (`services/ir_kakao.py`). `{달}`·`{기업들}` 은 자료라 코드가 채우고,
    # 나머지 문장은 팀이 화면에서 고친다. `{연월}`(`2026.09`)도 쓸 수 있지만
    # 기본 머리말은 더는 안 쓴다 — 사용자가 고른 제목에 연월 자리가 없다.
    #
    # **글자까지 `ir_kakao.DEFAULT_HEAD` · `DEFAULT_TAIL` 과 같아야 한다** —
    # 갈리면 문구틀을 지운 사람만 다른 글을 받는다(`tests/test_startup_ir_kakao`
    # 가 견준다).
    #
    # **이미 운영 DB 에 저장된 문구는 이 값을 바꿔도 안 바뀐다**(시드는 없을 때만
    # 넣는다). 그래서 지금 쓰고 있는 팀은 **옛 제목·옛 인사 그대로** 받는다 —
    # 새 모양을 받으려면 사람이 문구 화면에서 이 글로 고쳐야 한다(코드가 남의
    # 문장을 덮어쓰지 않는다). 한 대 더 옛것, 곧 맺음말이 머리말 한 덩어리에
    # 들어 있는 문구는 그 줄만 코드가 알아보고 목록 아래로 내려 준다
    # (`ir_kakao.split_lead`) — 같은 말이 두 번 적히지 않게 하려는 것이다.
    ("startup_sms", "[CONTACTVC ASSET]\n월간 IR 진행 현황\n\n안녕하세요 대표님.\n\n{기업들}의 {달} 말 기준 IR 진행 현황을 안내드립니다."),
    ("startup_sms_tail", "현재까지 위 투자사에 IR 자료가 전달되어 투자 검토가 진행 중입니다.\n\n향후 IR 미팅 요청이 발생하는 투자사가 있을 경우 별도로 안내드리겠습니다.\n\n감사합니다.\nCONTACTVC ASSET"),
