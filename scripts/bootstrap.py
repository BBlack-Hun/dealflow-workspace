...
TEAM_TEMPLATES = [
    ...
    # (`services/ir_kakao.py`). `{연월}`·`{달}`·`{기업들}` 은 자료라 코드가
    # 채우고, 나머지 문장은 팀이 화면에서 고친다.
    #
    ...
    # 넣는다). 저장돼 있는 옛 머리말은 맺음말 줄이 한 덩어리로 들어 있는데, 그
    # 줄은 코드가 알아보고 목록 아래로 내려 준다(`ir_kakao.split_lead`) — 같은
    # 말이 두 번 적히지 않게 하려는 것이고, 제목 두 줄은 **안 생긴다**. 제목까지
    # 받으려면 사람이 문구 화면에서 이 글로 고쳐야 한다.
    ("startup_sms", "[01. 월간 IR 진행 현황]\nCONTACTVC ASSET | {연월}\n\n안녕하세요 대표님\n{기업들} IR 자료 요청 현황을 보고드립니다."),
    ("startup_sms_tail", "미팅 요청 투자사가 나오면 별도로 연락드리겠습니다."),
