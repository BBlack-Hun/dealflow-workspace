...
TEAM_TEMPLATES = [
    ...
    # {기업목록} 은 "1번 기업 샘플애그" 처럼 **지난 회차에서의 번호**로 채워진다 —
    ...
    # 기업 리마인드 — **머리말만**이다. 그 아래 IR 자료를 요청한 투자사 목록은
    # 코드가 붙인다(`services/ir_kakao.py`). `{달}`·`{기업들}` 은 자료라 코드가
    # 채우고, 나머지 문장은 팀이 화면에서 고친다.
    #
    # 옛 시드는 목록이 없는 다른 글이었다(`최근 투자사에서 신규 기업 검토
    # 요청이 있어 …`) — 사람이 그 아래에 목록을 손으로 붙여 보내고 있었다.
    # **이미 운영 DB 에 저장된 문구는 이 값을 바꿔도 안 바뀐다**(시드는 없을 때만
    # 넣는다). 옛 글을 그대로 두면 목록 위에 옛 머리말이 붙으므로, 문구 화면에서
    # 사람이 고쳐야 한다.
    ("startup_sms", "안녕하세요 대표님\n{달} 말까지 {기업들}\nIR 자료 요청한투자사 리스트 입니다 미팅요청 투자사가 나오면 연락을 드릴예정 입니다."),
