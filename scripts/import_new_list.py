...
남는 위험은 둘이고 둘 다 화면에 적는다 — ① 같은 기업이 앱의 **다른 명단**에도
있으면 두 줄이 된다(대조가 거기까지 안 간다), ② 다음 판에서 시트의 기업명이
바뀌어 있으면 그 줄을 못 찾아 또 만든다. 시트에 번호를 채우면 둘 다 사라진다.
...
def main() -> int:
    ...
    if no_phone:
        ...
        print(f"  그중 번호 없이 들어간 줄 {len(no_phone)}개 "
              ...
              f"이었습니다. 시트에 번호를 채워 두면 앱 전체와 대조합니다.")
    db.close()
    return 0
