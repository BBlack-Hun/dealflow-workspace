...
NOTES = [
    ...
    # 사이에 서고, 달을 못 읽는 칸은 혼자 한 묶음이라 펴 둘 달 자리를 먹어
    # 그 달의 기록을 접어 버린다(`contact_columns.split_months` · 아래 `원본NO`).
    ...
    #
    # 여기서 안 잡으면 남는 머리글이라 월별 칸으로 선다(아래 `months`). 그런데
    # 스타트업 배치의 월별 칸은 `O`/`X` 고르기라 적힌 글이 한 번의 수정으로
    # 덮이고, 시트에서 이 둘이 앞에 적혀 있어 그 달의 기록을 접어 버린다
    # (`contact_columns.STARTUP_LAYOUT` 의 같은 자리 주석).
    #
    # `--map` 으로 부르는 쪽이 걸 수도 있었지만 여기 둔다 — 팀원들의 스타트업
    # 워크북이 **한 틀**에서 나와 머리글이 같다. 부르는 쪽에 맡기면 다음 사람의
    # 시트를 넣을 때 두 인자를 또 적어야 하고, 안 적은 한 번이 조용히 옛 동작을
    # 한다(이 파일 맨 위 "명단 이름은 인자로 받는다" 가 경계하는 그것이다).
    ("원본NO", "origin_no"),
    # 우리 팀 안에서 넘긴 이력이다. 스타트업 쪽 담당자가 아니라서 `성함`(name)
    # 으로 보내지 않는다 — 이 배치의 시트는 기업의 연락 상대를 `성함` 이라 적는다.
    ("담당자", "owner_history"),
    # 골라 넣는 칸 중 **달에 매이지 않는 셋**(제안서 ①·②·④ —
    # `contact_columns.STARTUP_LAYOUT` 의 같은 자리 주석).
    #
    # 아직 어느 시트에도 이 머리글이 없다. 그래도 여기 적어 두는 이유는 위
    # `원본NO` 와 같다 — 여기서 안 잡으면 **남는 머리글이라 월별 칸으로 선다.**
    # 달에 매이지 않는 값이 달 칸 자리에 서면 펴 둘 달 자리를 하나 먹어
    # 그 달의 기록을 접어 버리고, 달이 바뀔 때마다 지난달로 밀려 내려간다.
    # 화면에서 적기 시작한 값이 시트를 한 번 올리는 것으로 그렇게 갈린다.
    ("투자유치 상태", "funding_status"),
    ("당사 협업 상태", "collab_status"),
    # 무료 IR 미팅을 언제 제공하기로 했는가(`contact_columns.STARTUP_LAYOUT` 의
    # `이메일` 바로 뒤 칸). **아직 어느 시트에도 이 머리글이 없다** — 그래도
    # 여기 적는 이유는 바로 위 두 칸과 같다: 여기서 안 잡으면 남는 머리글이라
    # 월별 칸으로 서고, 달에 안 매이는 값이 달 칸 자리에 서면 펴 둘 달 자리를
    # 하나 먹어 그 달의 기록을 접어 버린다. 화면에서 적기 시작한 값이 시트를
    # 한 번 올리는 것으로 그렇게 갈린다.
    ("IR 미팅 제공일자", "ir_meeting_offered_at"),
    ...
    # 매이는 값이 달 칸 자리에 서면 펴 둘 달 자리를 하나 먹어 그 달의
