...
MENU = [
    ...
    # ── 명단 셋을 붙여 둔다: 투자사 · 스타트업 · IR 기업 ────────────────────
    #
    # 셋 다 **사람·기업 명단을 여는 화면**이고, 하루에도 몇 번씩 서로 오간다
    # (투자사에게 보낼 딜을 IR 기업 현황에서 고르고, 그 기업의 대표는 스타트업
    # 명단에 있다). 사이에 다른 메뉴가 끼면 그때마다 눈이 목록을 다시 훑는다.
    #
    # 순서는 **쓰는 빈도**다. 매일 보는 투자사 명단이 먼저, 달마다 챙기는
    # 스타트업이 다음, 자료를 채워 두고 꺼내 쓰는 IR 기업이 뒤다.
    {"key": "vc", "label": "투자사 관리 현황", "href": "/contacts", "ready": True},

    # 처음에는 참고 자료만 있던 화면이라 `스타트업 리마인드` 였다 — 무엇을 하는
    # 화면인지(달마다 한 번 챙긴다)를 이름에 남긴 것이다. 지금은 **명단이 여기
    # 산다**(`Layout.page`). 명단 화면의 이름은 옆의 `투자사 관리 현황`·`IR 기업
    # 현황` 처럼 **무엇의 명단인가**로 읽혀야 짝이 맞는다 — `리마인드` 가 붙어
    # 있으면 셋 중 이것만 일 이름이라 같은 줄에서 결이 어긋난다.
    {"key": "startup", "label": "스타트업", "href": "/startup", "ready": True},

    {"key": "su", "label": "IR 기업 현황", "href": "/companies", "ready": True},
    ...
    # `IR 기업 현황` 과 겹쳐 세 메뉴가 한 덩어리로 읽혔다 — 담당자를 가리키는
