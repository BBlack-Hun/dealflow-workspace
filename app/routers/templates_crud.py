...
KINDS = [
    ...
    # 이 문구는 **머리말만**이다 — 그 아래 IR 자료를 요청한 투자사 목록은 코드가
    # 붙인다(`services/ir_kakao.py`). 목록까지 여기서 적게 하지 않는 까닭은 그
    # 파일 머리말에 있다(짧게: 목록은 자료이고, 가리는 일이 문구틀 밖에 있어야
    # 한다). 비워 두면 코드에 적힌 머리말로 나간다.
    ("startup_sms", "기업 리마인드 — 문자",
     "IR 자료를 요청한 투자사가 있다고 스타트업에 알리는 문구 — "
     "여기는 머리말만 적고, 요청 투자사 목록은 앱이 붙입니다 "
     "({달}·{기업들} 은 앱이 채웁니다)"),
