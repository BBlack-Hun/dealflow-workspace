...
#
# ## 어디로 가나 — **시험방 하나뿐이다**  ★ 이것이 이 기능의 안전선이다
#
# 시험이 만드는 잡의 방 이름은 `config.TEST_ROOM` 이다. **사람이 고르지 못한다.**
# 방 이름을 적는 칸은 방 이름 시험에만 있고, 그쪽은 아무것도 보내지 않는다
# (검색해서 열리는지만 본다). 잡을 세우는 `_queue_test_job` 이 방 이름이 정말
# 시험방인지 한 번 더 본다 — 시험 문구가 진짜 투자사에게 가는 쪽이 훨씬 나쁘다.
#
# ## 시험방이 없으면 자리 자체가 없다
#
# 화면에서 감추기만 하면 주소로 그대로 부를 수 있으므로(이 저장소가 여러 번
# 겪은 사고다) 라우터도 같이 닫는다. 판정은 아래 함수 하나이고 화면·라우터가
# 그것을 함께 읽는다.
#
# ★ **예전에는 이 조건이 "운영에는 시험방이 없다" 를 뜻했다.** 그때는 시험방이
# 설정되는 순간 `create_send_list` 가 **모든 발송**을 그 방으로 돌렸기 때문에,
# 운영에 값을 넣는 것 자체가 사고였다. 지금은 아니다 — 일반 발송은 시험방을
# 읽지 않는다(`app/config.py: TEST_ROOM`). **운영에 시험방이 켜져 있어도 된다**
# 는 것이 이 변경의 목적이고, 그래서 이 자리는 운영에서도 열려 있을 수 있다.
# 열려 있어도 새지 않는 까닭은 가르는 선이 값이 아니라 **잡 종류**이기 때문이다
# (`models.TEST_SEND_KIND`).
#
# ## 왜 "임시" 로 두지 않았나
#
...
def _queue_test_job(db: Session, user: User, kind: str, room: str,
    ...
    **로그인한 사람 것으로 만든다.** 발송 잡은 그 사람의 기기 토큰으로만 내려가고
    ...
    ## ★ 시험 발송은 **시험방 말고는 어디에도 갈 수 없다**

    부르는 곳 넷이 전부 `_test_room()` 을 지나 방 이름을 받지만, 방 이름이
    **매개변수**인 이상 네 곳 중 하나가 언젠가 다른 값을 넘긴다 — 그날 지어낸
    보기 자료(`services/test_demo.py`)가 진짜 투자사 방으로 간다. 시험방이 실방
    글을 받는 것보다 **훨씬 나쁘다**: 투자사가 `[보기 자료] … 보기기업` 을 읽는다.
    그래서 잡을 세우는 이 한 곳에서 다시 못박는다.
    """
    room = (room or "").strip()
    expected = (config.TEST_ROOM or "").strip()
    if kind == TEST_SEND_KIND and room != expected:
        # 500 으로 터뜨린다. 조용히 고쳐서 보내면 **부르는 쪽의 실수가 묻힌다** —
        # 실수는 다음 갈래에서 다시 나오고, 그때는 막을 자리가 없을 수도 있다.
        raise RuntimeError(
            "시험 발송은 시험방으로만 갑니다 — 다른 방 이름이 넘어왔습니다")
...
def test_startup_remind(
    ...
    **진짜가 있으면 진짜를 쓴다.** 시험의 값어치가 거기 있다(담당자 성함이
    ...
    예전에 딜소개가 시험 모드에서 `[테스트 발송 → …]` 를 앞에 붙였다 — 모든
    발송이 한 방에 쏟아지던 때라 누구에게 갈 문구였는지 적어 둘 데가 거기밖에
    없었다. 그 장치는 없어졌다(일반 발송은 이제 제 방으로 간다). 여기는 사람이
    기업을 골라 한 통을 보내는 자리이고, **스타트업 화면이 내는 것과 글자
    하나까지 같은지**를 보는 것이 이 시험의 알맹이다 — 앞에 한 줄이라도 얹으면
    그것을 볼 수 없다. 지어낸 보기 자료로 떨어질 때만 표시 줄이 붙는다
    (`services/test_demo.py: MARK`).
