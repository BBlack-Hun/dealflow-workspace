...
def _eok(value):
    ...
    **숫자로 읽히면 숫자로 둔다** — 받아서 합계·평균을 내는 자리라, 옛 정수
    값에서 오던 `5.6` 이 그대로 숫자여야 한다. `5천만원` 처럼 억이 아닌 단위로
    적힌 값도 억으로 옮겨 `0.47` 같은 숫자로 나간다(머리글이 `(억)` 이다).
    ...
    **구간·`~`·못 읽는 글자는 적은 그대로** 내려간다(`5-10억 사이` · `~`).
    ...
    # **억으로 옮겨서 내보낸다.** 머리글이 `(억)` 이라 `5천만원` 을 그대로 실으면
    # 그 열만 단위가 달라진다 — 합계를 내는 사람이 100배를 헛짚는다.
    return float(amount.eok(value))


# ── ★ 화면 이름이 바뀌어도 **여기 머리글은 그대로 둔다** ─────────────────────
#
# 2026-09 에 화면 이름 넷이 바뀌었다(`사업분야 대분류`→`대분류` ·
# `기업구분`→`투자라운드` · `대표자`→`대표자명` · `기보, 신보, 중진공`→`정책자금`).
# 그중 이 목록에 있는 것은 `기업구분` 하나인데, **안 바꾼다.**
#
#   · 이 파일을 받아 쓰던 **수식이 머리글로 칸을 찾는다.** 바꾸면 그 수식이
#     말없이 빈 칸을 가리킨다 — 아래 `계약`·`계약월` 을 줄임말 그대로 둔 것과
#     같은 이유다.
#   · 이 목록은 애초에 **화면 이름을 따라가지 않는다.** `분야(대)`·`분야(소)`
#     는 화면에서 한 번도 그렇게 불린 적이 없고, `경쟁력` 도 화면에서는
#     `특이사항 (장점)` 이다. 화면 이름을 따르는 칸은 일부러 그렇게 적어 둔
#     둘뿐이다(`계약서 수신 여부` · `미팅제공일자` — 아래 주석).
#   · 엑셀은 **적는 자리가 아니라 내려받아 대조하는 자리**다. 화면에서 이름을
#     바꾼 까닭(입력하는 사람이 읽기 쉽게)이 여기에는 그대로 적용되지 않는다.
#
# 바꾸기로 정한다면 **받아 쓰는 수식을 먼저 찾아 고쳐야 한다** — 이 파일만
# 고치면 그쪽이 조용히 깨진다.
COMPANY_HEADERS = [
    ...
    # `계약서 수신 여부` 는 **화면 이름 그대로** 적는다. 내려받은 파일을 표와
    ...
    # `투자현황`(`funding_status`)은 **일부러 남겼다.** 그 칸은 [수정] 창에서
    # 뺐지만(`templates/companies.html` 의 `그 밖` 묶음 머리 주석) 값 281줄은
    # DB 에 그대로 두기로 한 것이라, **그 값을 읽을 수 있는 자리가 여기뿐이다.**
    # 남겨 둔 값을 못 읽으면 남겨 둔 뜻이 없다 — 엑셀은 적는 자리가 아니라
    # 내려받아 대조하는 자리라서, 화면에서 뺀 것과 어긋나지도 않는다.
    #
    # 이름도 안 바꿨다. 이 파일을 받아 쓰던 수식이 머리글로 칸을 찾는다
    # (`계약`·`계약월` 을 줄임말 그대로 둔 것과 같은 이유다).
    #
    # 언젠가 이 칸을 정말 버리기로 하면 **여기가 마지막 자리**다. 여기서 빼는
    # 순간 281줄은 DB 를 직접 여는 것 말고는 읽을 길이 없어진다.
    "경쟁력", "미팅제공일자", "계약", "계약서 수신 여부", "계약월", "탑딜", "투자현황", "요약상태",
