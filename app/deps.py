...
def _amount_warn(value) -> str:
    ...
    return ("숫자로 읽지 못해 딜소개 문구에서 빠집니다 — "
            "`18.3` · `5-10억` 처럼 적어 주세요. "
            "1억 미만은 `5천만원` · `4700만원` 으로도 적을 수 있습니다. "
            "값을 모른다는 뜻이면 `~` 로 적습니다.")
...
CONSULTANT_PATHS = (
    ...
    "/logout",
    "/account/password",
    # 공지를 **확인했다고 적는 길.** 판은 밑틀(`base.html`)에 서므로
    # 투자컨설턴트의 화면 위에도 뜬다 — 막으면 그 계정에서는 같은 공지가
    # 영영 안 닫힌다. 여는 것은 이 한 길뿐이고(적고 내리는 것은 `/team`),
    # 이 길이 하는 일은 **자기가 읽었다는 표시** 하나다.
    "/notices/seen",
