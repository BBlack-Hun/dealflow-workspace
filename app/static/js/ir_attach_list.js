...
// (`app/services/deal_numbers.py`), 투자사가 "기업2 주세요" 라고 답한 그
// 번호다. 담당자마다 다르다 — 같은 기업이 A 담당자에겐 2번, B 담당자에겐
// 5번일 수 있다. 그래서 **문구를 만든 그 함수**가 목록의 번호도 함께 낸다
// (`numbered_companies`). 화면이 따로 세면 목록은 `1`, 문구는 `[기업2] …` 가
// 되어 어느 쪽이 맞는지 알 수 없다.
//
// ## 적히는 **글자**도 서버가 지어 준다 (`attachments[].label`)
//
// 목록에 적는 것은 `[기업2]` — 나가는 문구가 기업을 짚는 바로 그 글자다
// (`deal_numbers.label`). 화면이 `no` 를 받아 제 모양으로 적으면(예전의
// `2번`) 문구의 모양을 바꿀 때 목록만 옛 모양으로 남아, 붙이는 사람이
// `[기업2]` 와 `2번` 을 머릿속에서 맞춰 봐야 한다. 그래서 **글자째 받아
// 적는다** — 여기서 짓는 모양은 하나도 없다.
//
// 차례는 **번호 오름차순**이고, 그 차례도 서버가 정해 실어 준다
// (`deal_numbers.numbered_companies`). 문구도 같은 함수에서 나오므로 둘이 갈릴
// 자리가 없다. **여기서 다시 정렬하지 않는다** — 화면이 또 세우면 서버가 어떤
// 차례로 보내든 화면만 달라져, 문구와 갈리는 자리가 하나 더 생긴다.
//
...
(function () {
  ...
  function renderList(list, note, preview) {
    ...
    items.forEach(function (a) {
      ...
      var badge;
      if (sample) badge = "";
      else if (a.label) badge = '<b class="ir-no">' + escapeHtml(String(a.label)) + '</b> ';
