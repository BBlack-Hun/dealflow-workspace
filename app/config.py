...
# ── 시험방 ────────────────────────────────────────────────────────────────────
# `/setup` 의 **시험 단추**가 보내는 곳. 카카오톡 '나와의 채팅' 방 제목을 적는다.
# 예: DEALFLOW_TEST_ROOM="본인 이름"
#
# ★ **일반 발송은 이 값을 읽지 않는다.** 딜 소개·IR 전달·미팅 요청·리마인드·
#   소싱·스타트업 월간은 이 값이 무엇이든 **각 담당자의 방**으로 간다
#   (`routers/deals.py: create_send_list`).
#
# 예전에는 반대였다 — 값이 있으면 **모든 발송**이 이 방 하나로 모였다. 그 장치는
# "운영에는 이 값이 없다" 를 전제로 했는데, 시험을 한 번 하려고 운영에 값을
# 넣어 두자 **딜 소개가 투자사 대신 이 방으로 갔다.** 투자사는 아무것도 못
# 받았고, 화면에는 '보냄' 으로 찍혔다.
#
# 이제 가르는 것은 **잡 종류**다(`models.TEST_SEND_KIND`). 시험 발송은 만드는
# 자리가 따로 있고(`routers/setup.py`), 그 자리만 이 값을 읽는다. 그래서 운영에
# 값이 남아 있어도 **새지 않는다** — 값을 지우는 것을 사람이 기억할 필요가 없다.
#
# 비워 두면 `/setup` 의 시험 자리 자체가 사라진다(화면도 라우터도).
#
# 리허설(실제 발송 길을 끝까지 걸어 보기)은 **담당자 줄의 방 이름을** 이 값으로
# 두어서 한다 — `scripts/rehearsal.py` 가 그렇게 만든다.
