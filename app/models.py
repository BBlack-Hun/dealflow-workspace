...
class IrCompany(TimestampMixin, Base):
    ...
    # `삭제 완료` · `8/13 삭제` 처럼 날짜가 아닌 메모다. 날짜 칸으로 못
    ...
    # `스타트업` 명단과 `투자컨설턴트` 가 **겹치지 않는다**(2026-09-09 에 개발
    # 자료로 세었을 때 0곳). 센 값이라 자료가 늘면 달라지지만, 애초에 이어 주는
    # 열쇠가 없다는 사실은 그대로다 — 서로 다른 기업 무리를 본다. 목록을
    # 잇지 않은 채 칸만 하나로 모으면 값이 옮겨지는 것이 아니라 갈 곳이
    # 없어 사라진다.
    #
    ...
    def introducible(self) -> bool:
        ...
        **문구는 이제 갈래가 둘이다**(`services/message_composer`):
        `딜 소개 문구`(`one_liner`)에 적어 둔 글이 있으면 **그것만** 나가고,
        비었을 때만 `[분야] | 매출 … | 누적투자 … | Pre Value …` 로 조합된다.
        아래 조건이 `분야 **또는** 문구` 인 한 **문구가 빈 채로도 소개 대상에
        선다** — 그 기업이 조합으로 나가는 쪽이다. 여기를 `문구 필수` 로
        조이면 조합 갈래가 영영 안 쓰이는 대신, 지금 소개되고 있는 기업이
        소리 없이 목록에서 빠진다(개발 사본에서 소개 가능 114곳 중 문구가 빈
        곳은 0곳이라 오늘 당장 달라질 것은 없지만, 금액 한 칸을 채우는 순간
        갈리는 판단이다). 바꾼다면 화면의 `내용 부족` 이유
        (`routers/companies.REQUIRED_FIELDS`)에 `딜 소개 문구` 를 함께
        넣어야 한다 — 지금 그 목록에는 이 칸이 없어서, 문구가 비었다고
        알려 주는 화면이 한 곳도 없다.
...
#:
#: ★ **이 값이 시험방과 실방을 가르는 선이다.** 시험방으로 가는 발송은 이
#: 종류뿐이고, 이 종류를 만드는 곳은 `routers/setup.py: _queue_test_job` 하나다
#: (거기서 방 이름이 `config.TEST_ROOM` 과 같은지 못박는다). 일반 발송을 만드는
#: `routers/deals.py: create_send_list` 는 시험방을 아예 읽지 않는다.
#:
#: 예전에는 가르는 선이 **`config.TEST_ROOM` 이 비었는가** 였다 — 값이 있으면
#: 모든 발송이 시험방으로 모였다. 운영에 값이 한 번 들어가자 딜 소개가 투자사
#: 대신 시험방으로 갔다. 종류로 가르면 값이 남아 있어도 새지 않는다.
#:
...
class SendItem(TimestampMixin, Base):
    ...
    # 이미 만들어 둔 발송은 그때 고른 자료를 보낸다 — 문구는 "[기업1] …" 이라고
...
class ConsultingColumn(TimestampMixin, Base):
    ...
    오래 `user_id` 를 달고 있었다. 컨설턴트가 한 사람뿐이던 동안에는 사람과 탭이
    일대일이라 아무 차이가 없었는데, **두 사람째가 들어오는 순간** 같은 탭에
    `8월 마지막주 리마인드 톡 or TEL` 이 사람 수만큼 서게 된다. 그 화면이
    이 표의 본래 자리다 — 팀이 컨설턴트들의 표를 `담당: 전체` 로 나란히 놓고
    보는 자리(`routers/consulting.py` 의 `scope`).

    거기서 벌어지는 일이 셋이다.

      · 머리글이 `8월 …` · `8월 …` 로 겹쳐 서고, 줄마다 **자기 담당의 칸에만**
        값이 있어 표의 절반이 늘 빈칸이다. 어느 칸이 내 칸인지 화면에 안 나온다.
      · `지난달 리마인드 미완료` KPI 가 거짓말을 한다. 지난달 칸을 이름으로
        찾는데(`_prev_month_label`) 먼저 걸린 **한 사람의 칸**만 보므로,
        나머지 담당의 줄은 전부 미완료로 세어진다.
      · 담당을 옮기면 그 줄의 기록이 통째로 안 보인다. `notes` 의 열쇠가
        옛 담당의 칸 id 인데 새 담당의 표에는 그 칸이 없다.

    그래서 **탭마다 한 벌**로 둔다. 탭 셋은 팀이 함께 쓰는 업무 단계이고
    (`ConsultingSheet` 참고) `마지막주 리마인드` 는 그 단계의 리듬이지 사람의
    취향이 아니다. 같은 모양을 옆 표가 이미 그렇게 풀어 두었다 —
    `ContactColumn` 은 처음부터 **명단마다**이고, 자동 생성도 명단 단위로 돈다
    (`services/monthly_columns.py` 의 `ensure_contact`).

    칸에 주인이 없으므로 **고치는 것은 관리자뿐**이다. 새 규칙이 아니라 이미
    있는 판정이 그대로 답한 것이다(`routers/consulting.py` 의 `may_edit_row` —
    주인 없는 것은 관리자만). 팀이 함께 보는 머리글을 한 사람이 지우면 그 달
    기록이 팀 전체에서 사라지므로 결과도 맞다.
    """

    __tablename__ = "consulting_columns"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    # 월 컬럼은 시트마다 다르다 — `스타트업` 은 6·7·8월, `경영본부 전달
...
class ConsultingCompany(TimestampMixin, Base):
    ...
    # 컨설턴트는 자기 줄만 보고, 전체를 보는 사람은 화면 위 `담당` 에서
    # 한 사람만 골라 본다(`routers/consulting.py` 의 `scope`).
    #
    # **담당이 갈리는 것은 줄이지 칸이 아니다.** 월별 리마인드 칸은 탭마다
    # 한 벌이라(`ConsultingColumn`) 담당을 바꿔도 같은 칸에 그대로 적힌다.
...
class ContactColumn(TimestampMixin, Base):
    ...
    `ConsultingColumn` 과 **같은 규칙**이다 — 둘 다 표(명단·탭)마다 한 벌이지
    사람마다가 아니다. 열을 정하는 것은 올린 사람이 아니라 원본 시트이고,
    명단·탭은 담당이 바뀌어도 같은 명단·탭이기 때문이다. 저쪽은 한동안
    사람마다였는데, 두 번째 컨설턴트가 들어오는 순간 같은 달 머리글이 사람
    수만큼 서는 것이 드러나 이쪽으로 맞췄다(`ConsultingColumn` 참고).
...
class AutoSendSetting(TimestampMixin, Base):
    """**결과 문의 대기 목록을 때맞춰 세워 둘지**를 정하는 줄. 종류마다 하나다.

    켜 두면 물어볼 때가 된 건을 모아 발송 목록이 저절로 선다. **보내는 것은
    사람이다** — 목록은 `draft` 로 서고, 그 계정이 진행 화면에서 [발송 시작] 을
    눌러야 나간다(`services/auto_send.py` 머리말).

    담는 것은 넷이다: 켜기/끄기 · **어느 계정 것으로 세우는가** · 언제 세우는가 ·
    하루 몇 건까지. **기본은 꺼짐**이고, 줄이 아예 없는 것도 꺼짐이다 — 아무것도
    안 넣으면 아무 일도 일어나지 않는다(메일·문자 설정이 그렇다).
    ...
    여기는 다르다. 목록은 팀 전체에서 **한 계정 것으로만** 서고, 그 계정이
    누구인지는 이 기능의 설정값 그 자체다. 계정 칸으로 두면 "켜짐인 계정"과
    "이 기능이 쓰는 계정"이 두 벌이 되어, 둘이 갈리는 날 누구 카톡에서 나갈지
    아무도 확신할 수 없다. 한 줄에 함께 둔다.

    ## 종류마다 한 줄인 이유

    지금 서 있는 종류는 미팅 후기(`meeting_review`) 하나다. 월말 기업 리마인드도
    같은 방식으로 세울 수 있어, 그때는 **줄 하나만 더 서면 된다** — 표도 실도
    화면도 그대로다(`services/auto_send.py` 머리말에 무엇을 더하면 되는지
    적어 두었다).
    """
    ...
    #: **어느 계정 것으로 세우는가.** 회차가 이 계정 것으로 서고, 그 계정의 기기
    #: 토큰으로만 내려간다(`routers/agent_api.py: poll`). 누르는 것도 그 계정이다.
    #: 비어 있으면 꺼짐과 같다 — 세워 줄 곳이 없으면 아무것도 못 한다.
    user_id: Mapped[Optional[int]] = mapped_column(ForeignKey("users.id"),
                                                   nullable=True)
    #: 몇 시부터 몇 시까지 목록을 세우는가(그 날의 지역 시각). 세운 뒤 사람이
    #: 곧바로 눌러 보내는 자리라, 받는 쪽(**투자사**)의 업무시간 안이어야 한다.
    #: 고를 수 있는 폭 자체를 코드가 좁혀 둔다
    ...
    #: **하루에 몇 건까지 세우는가.** 회당이 아니라 하루다 — 실이 30분마다
...
class AutoSendRun(TimestampMixin, Base):
    """**대기 목록에 세웠다는 표시.** 한 건에 한 줄, 영구히 남는다.

    ## 왜 줄을 남기나 — 두 번 세우면 두 번 나간다
    ...
    오늘`). 날짜를 열쇠에 넣으면 **다음 날 또 선다** — 실투자사에게 같은 문구가
    매일 갈 목록이 매일 만들어진다는 뜻이다. 그래서 `(kind, ref_id)` 하나로 잡고,
    그 자리는 영원히 비지 않는다.
    ...
    만든 뒤에 남기면, 목록은 섰는데 그 줄을 못 쓴 사이에 실이 다시 깨어나
    **한 벌 더** 세운다. 같은 사람에게 두 통이 실린 회차가 둘 서고, 사람이 둘 다
    누르면 두 번 간다. 그래서 먼저 자리를 잡고(`claimed`), 목록이 서면 그 줄을
    고친다(`ready`).

    목록을 못 세운 줄은 `failed` 로 남고 **다시 시도하지 않는다.** 조용히
    되풀이하는 것보다, 사람이 화면에서 그 사유를 보고 손으로 보내는 편이
    안전하다 — 그 건은 결과 문의 목록에 그대로 남아 있다.

    ## `sent` 를 두지 않는 이유

    이 표는 **두 번 세우지 않으려고** 있는 것이다. 실제로 나갔는지는 `send_items`
    가 안다 — 여기에 `sent` 를 두면 발송기가 꺼져 있어 한 통도 안 나간 회차를
    화면이 `보냄` 이라고 말한다(`deal_queue` 가 같은 이유로 `시작함` 이라고만
    적는다).

    ## 미팅 표에 칸을 붙이지 않은 이유

    `meetings.followup_done` 은 **물어보고 답을 들었다**는 뜻이다(무엇을 들었는지
    `followup_note` 에 적는다). 여기서 그 칸을 켜면 목록에서 사라져 아무도 답을
    적지 않는다.
    """
    ...
    #: 어느 계정 것으로 세웠나(그때의 설정값). 나중에 계정을 바꿔도 이 줄은
    #: 그날 누구 것으로 섰는지를 그대로 들고 있어야 한다.
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"))
    #: 하루 상한을 세는 열쇠. `YYYY-MM-DD`.
    day: Mapped[str] = mapped_column(String)
    #: 어느 발송 회차가 되었나. 화면이 `/jobs/{id}` 로 데려간다 — 누르기 전에
    #: **누구에게 어느 방으로 갈지** 사람이 봐야 하고, 누른 뒤에는 어디까지
    #: 갔는지를 그 화면이 이어서 말한다.
    job_id: Mapped[Optional[int]] = mapped_column(ForeignKey("send_jobs.id"),
                                                  nullable=True)
    #: claimed(자리를 잡았다) | ready(목록이 섰다) | failed(못 세웠다).
    #: 값은 `services/auto_send.py` 의 `STATUS_*` 한 곳에서 온다.
