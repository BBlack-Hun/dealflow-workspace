...
사용자가 **원하는 최종 모양을 그대로 적어 줬다.** 나가는 글은 이렇게 생겼다
(기업·투자사·사람은 여기 적을 수 없으니 지어낸 이름이다 — 이 저장소는 공개다.
목록은 자리만 보이게 **두 줄만 옮겼다** — 실제로는 `9개사` 만큼 선다).
...
샘플에너지의 9월 말 기준 IR 진행 현황을 안내드립니다.
...
────────────
05/06 가***파트너스 김*** 심사역
06/01 나***인베스트먼트 이*** 이사 / 한국공인회계사 / MBA
────────────

현재까지 위 투자사에 IR 자료가 전달되어 투자 검토가 진행 중입니다.

향후 IR 미팅 요청이 발생하는 투자사가 있을 경우 별도로 안내드리겠습니다.

감사합니다.
CONTACTVC ASSET
```

**한 번 더 바뀐 모양이다.** 앞선 모양(제목 `[01. …]` + `CONTACTVC ASSET |
2026.09`, 요약 한 줄 `■ 누적 2곳 · 9월 신규 0곳`, 자리 사이 두 칸, 맺음말 한
줄)은 우리가 몇 가지를 내놓고 사용자가 고른 것이었다. 이번 것은 **사용자가
직접 적어 온 글자**라, 고른 것보다 앞선다. 바뀐 자리는 여섯이다.

1. 제목 두 줄의 **차례가 뒤집혔다**. 보내는 곳 이름이 먼저이고, 글 제목이
   다음이다. `01.` 과 `| 2026.09` 는 빠졌다(연월 토큰은 남는다 —
   `YEARMONTH_TOKEN` 참고).
2. 요약이 **두 줄로 갈렸다.** 줄마다 제 이름표를 갖는다(`summary_line`).
3. 세는 단위가 **`곳` 에서 `개사`** 로 바뀌었고, 세는 것은 **투자사 수
   하나**다 — 곳과 명이 갈릴 때 둘 다 적던 규칙을 **지웠다**(`count_label`).
4. 요약과 구분선 사이, 구분선과 맺음말 사이에 **빈 줄**이 선다.
5. 목록 줄의 자리 사이가 **한 칸**이다(두 칸이었다 — `Line.text`).
6. 맺음말이 **한 줄에서 세 문단**이 됐다(`DEFAULT_TAIL`).

**가리기는 건드리지 않았다.** 사용자가 적어 준 보기의 목록 줄은 가리기가
들쭉날쭉한데(별표가 있는 줄과 없는 줄이 섞여 있다), 손으로 적다 생긴 차이인지
정말 원하는 것인지는 **따로 묻고 있다.** 답이 오기 전에 `ir_mask` 를 손대면
가려야 할 이름이 안 가려진 채 대표에게 나갈 수 있다 — 여기서 맞춘 것은 **줄의
짜임(날짜 · 투자사 · 사람 · 직함)과 칸 사이**뿐이다.

직함의 덧말(`/ 한국공인회계사 / MBA`)은 손댈 것이 없었다 — 직함은 처음부터
**원문 그대로** 나간다(`Line.title`).

**갈라 놓은 자리가 둘이다.** 사람이 고칠 **문장**은 문구틀에 있고
(제목 두 줄 · 인사 · 맺음말), **자료**는 코드가 짓는다(요약 숫자 · 목록 ·
구분선). 요약의 `9개사` 를 문구틀에 맡기면 사람이 손으로 적게 되고, 손으로 적은
숫자는 목록과 갈린다.
...
#:
#: ## `{연월}` 은 **기본 머리말이 더는 안 쓴다 — 그래도 남긴다**  ★
#:
#: 사용자가 고른 제목은 `[CONTACTVC ASSET]` / `월간 IR 진행 현황` 두 줄이라
#: `2026.09` 가 들어갈 자리가 없어졌다. 그렇다고 **치환을 지우면 안 된다**:
#: 운영 DB 에는 `{연월}` 이 적힌 옛 머리말이 그대로 저장돼 있고(시드는 없을
#: 때만 넣는다), 치환이 사라지면 그 팀의 대표에게 `{연월}` 이라는 **글자가
#: 그대로** 나간다. 이 저장소가 이미 겪은 사고다(`header` 의 `{담당자명}`).
#: 안 쓰는 토큰이 남는 값은 설명 한 줄이고, 지우는 값은 대표에게 나가는 글이다.
#: 팀이 제목에 연월을 다시 넣고 싶으면 문구 화면에서 적기만 하면 된다.
MONTH_TOKEN = "{달}"
YEARMONTH_TOKEN = "{연월}"
COMPANIES_TOKEN = "{기업들}"
...
#: 들어가고, 번호를 붙일지도 팀이 정한다). 요약 숫자만 코드가 짓는다.
#:
#: **바뀐 글이다.** 사용자가 원하는 최종 모양을 그대로 적어 줬다 — 제목의
#: 차례가 뒤집혔고(회사 이름이 먼저, 글 제목이 다음), `01.` 과 `| 2026.09` 가
#: 빠졌고, 문단마다 빈 줄이 섰고, `{기업들}` 이 **문장 가운데**로 들어갔다.
DEFAULT_HEAD = "\n".join([
    "[CONTACTVC ASSET]",
    "월간 IR 진행 현황",
    "",
    HELLO,
    "",
    f"{COMPANIES_TOKEN}의 {MONTH_TOKEN} 말 기준 IR 진행 현황을 안내드립니다.",
])

#: 맺음말 문구가 비어 있을 때 쓰는 글. 시드와 같은 글이다.
#:
#: **한 줄이 아니라 세 문단이 됐다**(사용자가 적어 준 모양이다). 마지막 두 줄은
#: 서명이라 빈 줄 없이 붙는다 — `감사합니다.` 와 보내는 곳 이름은 한 덩어리다.
DEFAULT_TAIL = "\n".join([
    "현재까지 위 투자사에 IR 자료가 전달되어 투자 검토가 진행 중입니다.",
    "",
    "향후 IR 미팅 요청이 발생하는 투자사가 있을 경우 별도로 안내드리겠습니다.",
...
class Line:
    ...
    **기업 자리는 글 하나에 기업이 여럿일 때만 찬다.** 기업이 하나면 그 이름은
    ...
    def text(self) -> str:
        ...
        #
        # ## 자리 사이는 **한 칸**이다 — 바뀐 규칙이다
        #
        # 예전에는 두 칸이었다(자리가 자리로 보이게 하려던 것이다). 사용자가
        # 원하는 최종 모양을 적어 주면서 **한 칸**으로 정했다
        # (`05/06 가***파트너스 김*** 이사`).
        #
        # 두 칸이 하던 일은 애초에 잘 되지 않았다: 카톡 글꼴은 글자폭이 일정하지
        # 않아서 칸을 늘려도 뒷자리가 줄마다 다른 데서 시작한다. 그러면서 줄만
        # 길어져 좁은 화면에서 줄바꿈이 생겼다 — 줄이 접히면 자리는 더 안 보인다.
        # 날짜를 두 자리로 맞춰 둔 것(`day_label`)은 그대로다.
        #
        # 자리 사이와 이름·직함 사이가 이제 같은 한 칸이라, 이어 붙이는 자리도
        # 하나로 뒀다 — 두 벌로 두면 한쪽만 고쳐질 자리가 생긴다.
...
class KakaoMessage:
    ...
    summary: str = ""           # 요약 **두 줄**(`■ 누적 …`). 화면이 그대로 보여 준다
    tail: str = ""              # 맺음말 — **목록 아래**에 붙는다(여러 줄)
...
def ym_label(month: str) -> str:
    """`2026-07` → `2026.07`. `{연월}` 이 이것으로 바뀐다.

    여기는 **두 자리**다(`07`). 목록 줄의 날짜와 같은 까닭이고, 달마다 줄의
    길이가 달라지지 않아야 해서다. 머리말 문장 안의 `7월`(`month_label`)
    과는 쓰는 자리가 다르다.

    **기본 머리말은 이제 이것을 안 쓴다**(제목에 연월 자리가 없어졌다). 그래도
    남아 있는 까닭은 `YEARMONTH_TOKEN` 에 적혀 있다 — 옛 문구를 쓰는 팀의
    글에서 `{연월}` 이 글자 그대로 나가면 안 된다.
    """
    if not ir_monthly.is_month(month):
        return ""
...
def count_label(firms: int) -> str:
    """`9개사` — **투자사 수만** 적는다.

    ## 줄 수와 안 맞을 수 있다 — **그대로 둔다**  ★

    목록 줄은 **사람마다** 하나씩 선다(`ir_monthly` 가 투자사가 아니라 사람으로
    묶는다). 그래서 한 투자사에서 두 분이 물어보면 `9개사` 라고 적힌 글에 목록
    줄이 10개, 11개 설 수 있다.

    **고장이 아니다. 사용자가 알고 고른 것이다.** 두 수가 갈릴 때 `9개사 (11명)`
    처럼 사람 수를 덧붙이는 길을 한 번 만들었다가 **지웠다** — 사용자가 "투자사
    수만 적는다" 로 정했다. 안 쓰는 길을 남겨 두면 언젠가 되살아난다.

    그러니 다음 사람에게 미리 적어 둔다: **여기서 `people` 을 받아 수를 맞추려
    들지 말 것.** 대표가 묻는 것은 "몇 개 투자사가 우리 자료를 봤나" 이고, 줄이
    그보다 많은 것은 같은 투자사에서 두 분이 봤다는 뜻이다. 셈을 맞추겠다고
    줄을 투자사로 묶으면 그 사실이 목록에서 사라진다.

    단위가 `곳` 이 아니라 `개사` 인 것도 사용자가 적어 준 모양이다. 요약 줄의
    이름표(`누적 IR 자료 요청 투자사 : `)가 세는 것이 무엇인지 줄 안에 말한다.
    """
    return f"{firms}개사"
...
def summary_line(month: str, tally) -> str:
    ...
    사용자가 적어 준 모양이다. 예전의 `■ 누적 2곳 · 9월 신규 0곳` 은 가운뎃점
    하나에 서로 다른 두 수가 걸려 있어서, 좁은 화면에서 줄이 접히면 어디서
    갈리는지가 기기마다 달랐다. 줄을 아예 둘로 세우면 어느 화면에서나 같게
    읽히고, 각 줄이 **제 이름표**를 갖는다(`누적 … 투자사` · `9월 신규 요청`).

    이름이 `summary_line` 인데 두 줄인 것은 부르는 자리가 여럿이라 그대로 뒀다 —
    이 글에서 **요약 덩어리**를 뜻하는 이름이고, `KakaoMessage.summary` 도 같다.

    ## 이 수는 **목록 줄 수와 다를 수 있다**

    한 투자사에서 두 분이 물어보면 `9개사` 아래 줄이 10개 선다. 사용자가 알고
    고른 것이라 맞추지 않는다 — 까닭은 `count_label` 에 적어 뒀다.
    """
    if tally is None or not tally.people:
        return ""
    piled = count_label(tally.firms)
    fresh = count_label(tally.new_firms)
    return "\n".join([
        f"{SUMMARY_MARK} 누적 IR 자료 요청 투자사 : {piled}",
        f"{SUMMARY_MARK} {month_label(month)} 신규 요청 : {fresh}",
    ])
...
def header(month: str, companies: Sequence[str], body: str = "",
    ...
    ## 토큰은 **문장 어디에 있어도 된다** — 글자 그대로 바꿔치기다  ★

    새 머리말은 `{기업들}의 {달} 말 기준 …` 처럼 토큰을 **문장 가운데**에
    둔다. 되는 까닭은 여기가 `str.replace` 이기 때문이다 — 줄 앞인지 뒤인지,
    뒤에 조사(`의`)가 붙는지를 보지 않는다. `render_template` 쪽도 같은 방식
    이라(`message_composer`), 두 단계 어디에도 자리를 가리는 규칙이 없다.
    `_fix_honorific` 만 글자를 건드리는데 그것은 `님` 앞의 공백뿐이라,
    `{기업들}의` 나 `대표님.` 에는 걸리지 않는다.

    **조사는 코드가 고르지 않는다.** `샘플에너지의` 는 늘 `의` 로 맞지만,
    `가/이` 처럼 앞 글자의 받침에 따라 갈리는 조사를 코드가 골라 주기 시작하면
    문구를 고치는 사람은 어떤 조사가 나올지 알 수 없게 된다. 문구에 적힌
    조사가 그대로 나간다.
    """
...
def pack(head: str, lines: Sequence[str], tail: str = "",
    ...
    ### 아래 구분선과 맺음말 사이에 **빈 줄**이 선다

    사용자가 적어 준 모양이다. 맺음말은 목록이 아니라 맺는 말이라, 구분선에
    바로 붙으면 목록의 꼬리처럼 읽힌다. 자리를 잴 때도 그 빈 줄을 **같이
    센다**(`+ 2`) — 안 세면 한 글자 차이로 마지막 통이 한계를 넘는다.
    ...
    if cur:
        parts.append(cur)
    if tail:
        if parts and len(parts[-1]) + 2 + len(tail) <= limit:
            parts[-1] = f"{parts[-1]}\n\n{tail}"
        else:
            parts.append(tail)
    return parts
...
def assemble(db: Session, user: Optional[User], month: str,
    ...
    **제목**이 됐지만, 표시는 여전히 그보다 앞이다 — 가짜라는 표시가 제목 아래로
    ...
    사용자가 적어 준 모양이다 — 머리말 / 요약 / 목록 / 맺음말이 전부 빈 줄로
    갈린다. 예전에는 요약과 구분선이 **붙어** 있었는데(`요약\n구분선`), 그러면
    요약 줄이 목록의 머리글처럼 읽혀 `■` 두 줄과 날짜 줄들이 한 덩어리로 보였다.

    위쪽 빈 줄은 여기서, 아래쪽(구분선과 맺음말 사이)은 `pack` 이 넣는다. 갈라
    둔 까닭은 통 나누기다: 아래 빈 줄은 맺음말이 마지막 통에 **들어가는지 재는**
    셈에 걸려 있어서, 그 셈을 하는 자리에 있어야 한다.
    """
