...
그래서 **사람이 적은 글자를 그대로 저장한다.** 단위는 **억**이다 — 표 머리글도
수정 창(`단위: 억`)도 엑셀 머리글(`누적투자(억)`)도 억이라, 사람이 보는 단위와
저장하는 단위가 같아야 `5-10억` 을 적었을 때 곱하거나 나눌 자리가 생기지 않는다.
곱할 자리가 있으면 `5-10억` 같은 값에서 그 자리가 막히고, 막힌 자리를 우회하는
두 번째 길이 생긴다.
