...
# ── 표에 한 번에 펴 둘 **달 수**. 이 저장소에서 한 곳이다 ───────────────────
#
# 달마다 칸이 늘어나는 표가 셋인데(투자컨설턴트 · 스타트업 리마인드 · 투자사
# 딜공유) **"몇 달치를 보는가" 는 세 표가 같이 정하는 한 가지 결정**이다.
# 그냥 두면 한 해 뒤에 열두 달치가 되어 가로로 밀어야 읽히고, 실제로 챙기는
# 것은 최근 몇 달뿐이다 — 그 "몇 달" 이 표마다 다를 이유가 없다.
#
# **왜 여기인가.** 접는 자리가 두 곳이다(`routers/consulting._split_columns` ·
# `services/contact_columns.split_months`). 두 곳이 각자 숫자를 적어 두면 한쪽만
# 고쳐지는 날이 오고, 그때 두 표는 **같은 화면에서 서로 다른 달 수**를 보여
# 준다. 두 곳이 이미 함께 쓰는 모듈은 여기뿐이라(둘 다 `month_of` 로 달을
# 읽는다) 숫자도 여기 둔다.
#
# **3 인 이유.** 투자컨설턴트가 먼저 석 달이었다 — 위 KPI 가 **지난달** 빈칸을
# 세므로 그 달이 표에 보여야 한다. 스타트업 리마인드도 사용자가 같은 것을
# 요청했다("스타트업도 3개월치 리마인드 메뉴가 나올 수 있고"). 한동안 스타트업
# 쪽만 1이었는데, 그때 근거는 "한 달에 세 칸씩 붙어 두 달치면 여섯 칸" 이었다.
# 넓어지는 것은 그대로 사실이라 **접는 길은 남아 있다**(`?months=all` 의 반대
# 방향 — 접힌 칸은 값이 그대로 남고 수정창에서 계속 보인다).
#
# 넘어간 달의 값은 **지워지지 않는다.** 접기는 표에 안 세우는 일일 뿐이고,
# 수정창은 접힌 달까지 전부 편다(`contact_columns.panel_columns` · 거기에
# `routers/pages.py` 가 `all_months` 를 준다).
VISIBLE_MONTHS = 3
