...
딜소개 문구는 `[기업1] …` `[기업2] …` 로 나가고 투자사는 **그 번호로 기억해서**
답한다("기업2 자료 주세요"). 그래서 자료를 보낼 때도 같은 번호로 짚어 줘야 서로
같은 기업 이야기를 한다. 번호를 새로 매기면 받는 쪽은 자기 목록에서 찾다가
못 찾는다.
...
쓰이는 자리는 셋인데, 셋이 따로 적혀 있으면 한쪽만 낡는다.
  ...
  · 나가는 문구의 `[기업1] [기업2] [기업3]`  (`message_composer.compose_message`)
  · 회차에 남는 번호                     (`DealBatchCompany.position`)
  · 자료 전달의 `[기업2] …`               (`company_list`)
...
화면이 따로 세면 문구가 짚은 것과 다른 자료가 붙는다. 모양까지 같이 나가는
것도 그래서다 — 문구가 `[기업2]` 라고 짚는데 목록만 `2번` 이면, 붙이는 사람이
두 표기를 머릿속에서 맞춰 봐야 한다.

## 번호의 **모양**도 한 곳이다 (`label`)

번호가 글자가 되는 자리는 셋이다 — 딜 소개 문구, 자료 전달 문구, [보낼 자료]
목록. 셋이 각자 모양을 적으면 한쪽만 낡아 **같은 것을 두 이름으로 부른다**
(딜 소개는 `[기업2]`, 자료 전달은 `2번 기업`). 그래서 모양을 짓는 자리는
`label()` 하나이고, 화면도 서버가 지어 준 그 글자를 그대로 받아 적는다
(`/api/deals/preview` 의 `attachments[].label`).
...
def label(no: int) -> str:
    """번호를 사람이 읽는 **한 가지 모양**으로 — `[기업2]`.

    딜 소개 문구도, 자료 전달 문구도, 화면의 [보낼 자료] 목록도 이 함수가 지은
    글자를 쓴다. 모양을 각자 적으면 한쪽만 낡아 **같은 기업을 두 이름으로**
    부르게 된다(`[기업2]` 와 `2번 기업`) — 투자사는 받은 문구의 그 글자로
    기억하고 답하므로, 자료를 보낼 때 다른 글자로 짚으면 되짚어 찾아야 한다.

    번호를 **정하는** 규칙은 여기서 손대지 않는다(`numbered`) — 이 함수가 하는
    일은 이미 정해진 번호를 글자로 적는 것뿐이다.
    """
    return f"[기업{no}]"


#: 사람이 손으로 적어 넣은 번호를 읽는 자리(`parse_label`). 모양을 짓는 곳과
#: 읽는 곳이 붙어 있어야, 모양을 바꿀 때 읽는 쪽이 같이 바뀐다.
_LABEL_RE = re.compile(r"^\[?\s*기업\s*(\d+)\s*\]?$")


def parse_label(token: str) -> Optional[int]:
    """사람이 적어 넣은 조각이 **번호면** 그 번호 — 아니면 `None`.

    받는 모양은 셋이다: `2` · `기업2` · `[기업2]`. 투자사가 답으로 말하는 것은
    문구에 적힌 그 글자라("기업2 주세요"), 받아 적는 사람도 그 글자를 그대로
    친다. 숫자만 받으면 그 줄이 **기업 이름으로 읽혀** `기업2` 라는 이름의
    요청이 남는다.

    숫자만 적던 길은 그대로 둔다 — 옛 문구(`2) …`)를 받은 투자사는 여전히
    "2번 주세요" 라고 답한다.
    """
    t = (token or "").strip()
    if t.isdigit():
...
def numbered(items: Sequence) -> List[Tuple[int, object]]:
    ...
    나가는 문구의 `[기업1] [기업2] [기업3]` 도, 회차에 남는 `position` 도 여기서
    나온다. 두 곳이 각자 세면 문구의 번호와 남은 번호가 갈리고, 그러면 다음에
    자료를 보낼 때 엉뚱한 기업을 짚는다.
    """
...
def numbered_companies(db, contact_id: int,
    ...
    본다.** 화면이 따로 세면 목록에는 `[기업2]`, 문구에는 `[기업3] …` 이 되어
...
def company_list(db, contact_id: int, companies) -> str:
    """'[기업1] 샘플애그' · 여럿이면 '[기업1] 샘플애그, [기업3] …'.

    문구틀 안에 **문장으로 들어간다**(`{기업목록} IR deck 먼저 전달드리겠습니다.`).
    그래서 번호는 이름 앞에 붙는 꼬리표로 두고 이름을 문장의 주어 자리에
    남긴다 — `[기업1] 샘플애그 IR deck 먼저 전달드리겠습니다.`
    ...
    """
    return ", ".join(
        f"{label(no)} {company.name}" if no else company.name
