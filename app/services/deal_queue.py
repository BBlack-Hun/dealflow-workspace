...
## 여기에 시각 칸을 두지 않는다 — 예약은 **회차에 붙는다**

이 줄에는 받는 사람도 문구 스냅숏도 없다(위). 시각을 여기 달아 두면 그 시각에
**목록을 만드는 일부터** 저절로 하게 되는데, 그러면 누구에게 무엇이 나가는지를
아무도 안 본 채로 발송이 시작된다 — 이 파일이 통째로 막고 있는 그 일이다.

예약 발송은 [시작] 을 눌러 **만들어진 회차**에 붙는다
(`services/scheduled_send.py`). 사람이 목록을 보고 세운 뒤에, 그것이 언제 나갈지만
정하는 것이다.
"""
