...
쓰이는 것은 옆 탭의 한 칸이라, 같은 내용을 **두 번 적고 있었다.**
...
SOURCE_FIELDS = [
    ...
    # 하나는 갈래(`헬스케어`)고 이 칸은 문장이다. 그래서 한동안 `사업 설명`
    # 이라 불렀는데, 그때는 이 칸이 표 어디에도 없어 맞출 상대가 없었다.
    # 이제 **스타트업DB 표의 `기업 한줄 소개` 가 곧 이 칸**이라(0058) 화면
    # 이름을 그것으로 맞춘다. 두 벌로 두면 화면과 이 목록이 갈린다.
