...
`funding_status`(투자현황)는 **쓰지 않는다.** (그 칸은 이제 화면에도 없다 —
`app/models.py` 의 그 칸 주석. 아래 이유는 왜 그런 칸이 되었는지까지 말해 준다.) 이름만 보면 '진행 상태' 같지만
...
SOURCE_FIELDS = [
    # 이름이 `사업분야` 였다. 그런데 IR 기업 현황에 `대분류`(`sector_major`
    # — 2026-09 까지 이름이 `사업분야 대분류` 였다)가 따로 있어서 **같은 말이 서로 다른 두 칸**을 가리켰다 —
...
def compose_one_liner(company) -> str:
    ...
    **빈 칸은 토막째 빠진다.** 실데이터는 대부분 일부만 차 있어서(누적투자금액은
    ...
    그래서 항목마다 '앞에서 이미 말했는가'를 보고 없을 때만 덧붙인다.

    **이 겹침 판정은 이제 여기에만 있다.** 딜 소개 문구
    (`message_composer.auto_company_summary`)에도 같은 검사가 한 벌 더 있었는데,
    그쪽은 갈래째 없앴다 — 사람이 `딜 소개 문구` 에 적어 둔 글이 있으면 아무것도
    덧붙이지 않으므로 판정할 것이 없다.

    **여기서는 없앨 수 없다.** 두 함수는 이름만 닮았지 하는 일이 다르다.
      · 저기는 **나가는 문구**를 정한다. 첫 토막이 사람이 그 칸에 적어 둔
        완성 문장이라, 뒤에 무엇을 붙일지 물을 것 없이 그대로 내보내면 된다.
      · 여기는 **제안을 만든다**(`[자동 조합]` 을 눌렀을 때만 칸에 들어간다,
        #203). 첫 토막은 스타트업DB 의 `기업 한줄 소개` 이고, 그 칸에는 재무가
        통째로 적혀 온 값이 실제로 들어 있다 — 그대로 이어 붙이면 사람이
        누르기도 전에 숫자가 두 번 적힌 제안이 만들어진다.
    즉 저쪽에서 검사가 없어진 것은 '덧붙이기를 그만둬서' 지 '겹침이 없어져서'
    가 아니다. 여기서 계속 덧붙이는 한 이 검사는 남는다.
    """
