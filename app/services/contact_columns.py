...
class Layout:
    ...
    month_seed: tuple = ()
    # 달마다 늘어나는 칸의 **보기를 이름으로** 정한다: `((찾을 말, 보기), …)`.
    #
    # **왜 이름으로 고르나.** 위 `month_choices` 는 배치에 하나뿐인데, 스타트업
    # 리마인드의 월별 칸은 성격이 셋으로 갈린다 — 문자를 보냈는가(③) · 통화가
    # 어떻게 됐는가(⑤) · 카톡방이 연결됐는가(⑥). 셋에 같은 보기를 주면 `통화
    # 완료` 와 `회신 받음` 이 한 목록에 섞여, 무엇을 고른 것인지 세지 못한다.
    #
    # **칸마다 손으로 붙이지 않는다.** 월별 칸은 달마다 새로 생기므로
    # (`services/monthly_columns.py`), 붙이는 것이 사람 손이면 다음 달 칸에는
    # 아무도 안 붙인다 — 그 달만 조용히 자유 입력으로 돌아가고 필터도 빠진다.
    # 대신 **이름으로** 고른다: 새 달 칸은 직전 달 칸에서 달 숫자만 바꿔
    # 만들어지고(`monthly_columns.relabel`), 밑자리도 같은 뒷말로 심으므로
    # (`month_seed`) 뒷말은 달이 바뀌어도 그대로다.
    #
    # 찾는 말은 **포함**이고 위에서부터 먼저 걸리는 것을 쓴다. 어느 것에도 안
    # 걸리는 칸은 `month_choices` 로 물러난다 — 시트에서 딸려 온, 팀이 정한
    # 세 모양이 아닌 칸(`리마인드 카톡(월1회-…)`)에 남의 보기를 붙이면 그 칸의
    # 옛 글이 무슨 뜻인지 읽는 사람이 잘못 짚는다.
    month_picks: tuple = ()
...
# ③ 그 달에 문자를 보냈는가 — **고른 달 기준**이라 월별 칸이다.
#
# 기존 문자 칸에 적혀 있던 `연결` 은 여기가 아니라 ④ 로 간다. 그것은 보냈다는
# 기록이 아니라 **보낸 뒤에 돌아온 반응**이다.
SEND_CHOICES = "미발송,발송 예정,발송 완료,발송 실패,발송 제외"
# ④ 문자·카톡 등 연락 이후의 반응. **달에 매이지 않는다** — 지금 이 기업이
# 어느 상태인지를 적는 칸이라, 달이 바뀐다고 `회신 받음` 이 사라지지 않는다.
#
# `회신 받음` 을 고른 뒤 **구체적인 답변은 메모에** 남긴다. 답변을 보기로
# 만들면 목록이 줄 수만큼 늘어나 다시 고를 것이 없는 칸이 된다.
REPLY_CHOICES = "회신 대기,회신 받음,재연락 요청,진행 거절,회신 불필요"
# ⑤ 통화가 어떻게 됐는가 — 기존 `TEL` 칸.
#
# **`통화 완료` 는 `진행 의사 있음` 이 아니다.** 전화가 닿았다는 것과 하겠다는
# 답을 들었다는 것은 다른 말이고, 뒤엣것은 ④(회신 상태)와 ②(당사 협업 상태)가
# 적는다. 한 칸으로 합치면 "통화는 됐는데 거절" 을 적을 자리가 없다.
CALL_CHOICES = ("미시도,통화 예정,통화 완료,부재중,통화 중 / 재시도 필요,"
                "재통화 요청,연락처 오류,통화 거절")
# ⑥ 카톡방이 연결됐는가 — 기존 `카톡 연결` 칸.
#
# 제안서는 이 칸에 `카톡방 참여여부` 를 **합치자**고 한다. 지금도 합치지
# 않는다 — 저쪽은 줄마다 하나뿐인 값(`VcContact.kakao_joined`)이고 이쪽은
# 달마다 하나씩이라, 합치는 순간 어느 달 값이 남는지를 앱이 정하게 된다.
# 무엇이 겹치고 무엇이 부딪히는지는 사람이 보고 정할 일이다.
#
# **그 줄마다 하나뿐인 값은 이제 표에도 선다** — `STARTUP_LAYOUT.tail` 의
# `카톡 연결 여부`. 그래서 이 표에는 카톡방을 적는 칸이 두 갈래로 선다:
# 이 월별 칸은 **그 달에** 연결이 어떻게 됐나(`9월 카톡 연결`)고, 저 붙박이
# 칸은 **지금 연결돼 있나**다. 둘은 저장 자리가 달라(`notes[c…]` vs 모델 칸)
# 서로 덮지 않는다 — 한쪽을 고쳐도 다른 쪽은 그대로다.
#
# `대표님이 초대해주심` 같은 말은 보기가 아니라 **메모나 활동 이력**으로 간다.
# 누가 초대했는지는 상태가 아니라 그때 있었던 일이다.
KAKAO_CHOICES = ("미연결,초대 요청,초대 완료 / 입장 대기,연결 완료,"
...
STARTUP_LAYOUT = Layout(
    ...
    # 월별 칸 셋의 보기. **`month_seed` 와 같은 뒷말로 찾는다** — 밑자리를 심는
    # 말과 보기를 고르는 말이 같아야, 새 명단에 처음 선 칸에도 그 자리에서
    # 보기가 붙는다. 아래 검사가 두 목록이 갈리지 않는지 잰다
    # (`tests/test_startup_status_choices.py`).
    #
    # `리마인드 문자` 가 아니라 `문자` 로 찾는 이유: 시트마다 앞말이 조금씩
    # 다르다(`9월 문자` · `7월 리마인드 문자 (7/28)`). 뒷말 한 낱말이 그 칸이
    # 무엇을 적는 칸인지를 이미 가리킨다.
    #
    # `카톡` 만으로 찾지 않는다 — 한 명단에 `리마인드 카톡(월1회-수or목or금)`
    # 이 서 있는데 그것은 **보내는 칸**이라 카톡방 연결 상태가 아니다.
    month_picks=(("문자", SEND_CHOICES),
                 ("TEL", CALL_CHOICES),
                 ("카톡 연결", KAKAO_CHOICES)),
    head=[
        ...
        # — **회신(④) 바로 뒤**라, "회신 왔나 → 그 달에 뭘 했나" 까지는 이어서
        # 읽힌다.
        #
        ...
        Column("계약서 수신여부", "invoice_received", 140, source="note",
               kind="pick", choices="O,X"),
        # ④ 연락 이후의 반응. ⑤(통화 결과)와 **다른 칸이다** — 전화가 닿은
        # 것과 하겠다는 답을 들은 것은 같은 말이 아니다(`CALL_CHOICES` 주석).
        #
        # **월별 묶음 바로 앞**이다. 이 값은 달에 매이지 않으므로 달 칸 사이에
        # 세우면 달이 바뀔 때 지난달 것으로 접혀, 지금 이 기업이 어느 상태인지를
        # 표에서 읽을 수가 없다. 그러면서도 ③⑤⑥ 과 이웃해야 "회신 왔나 → 그
        # 달에 문자·통화·카톡방은 어떻게 됐나" 가 한자리에서 읽힌다.
        # (`회신 상태 (1) ▾` = 105px)
    ...
    tail=[
        ...
        # ## 왜 여기인가
        #
        # 사용자가 `IR 자료 회신 여부` 바로 앞이라고 했고, 그 자리가 마침
        ...
        # `KAKAO_CHOICES`(여덟 가지)는 **달마다의** 월별 칸 것이라 여기 붙이면
        # 같은 표의 두 칸이 다른 말로 같은 것을 적게 된다. 투자사 표가 이 칸에
        # 이미 `O,X` 를 쓰고 있으므로 그것을 따른다 — 목록에 없는 옛 값은
        # `<datalist>`·`startPick` 이 그대로 살려 둔다.
        #
...
def month_choices(layout: Layout, label: str) -> str:
    """이 월별 칸의 보기 — **칸 이름으로 고른다.**

    같은 명단의 월별 칸이라도 적는 것이 다르다: `9월 리마인드 문자` 는 보냈는지
    (③), `9월 리마인드 TEL` 은 통화가 어떻게 됐는지(⑤), `9월 카톡 연결` 은
    카톡방이 연결됐는지(⑥)다. 셋에 한 목록을 주면 `통화 완료` 와 `회신 받음` 이
    한 칸에 섞여 세지 못한다.

    **이름으로 고르는 것이 핵심이다.** 월별 칸은 달마다 새로 생기는데
    (`services/monthly_columns.py`), 칸마다 사람이 붙이는 구조면 다음 달 칸에는
    아무도 안 붙인다 — 그 달만 조용히 자유 입력으로 돌아가고 필터도 빠진다.
    새 칸은 직전 달 칸에서 **달 숫자만** 바뀌어 나오고(`relabel`) 밑자리도 같은
    뒷말로 심기므로(`Layout.month_seed`), 뒷말은 달이 바뀌어도 그대로다. 그래서
    뒷말을 보면 새로 생긴 칸에도 그 자리에서 같은 보기가 붙는다.

    `TEL` 은 대소문자를 안 가린다 — 시트에 `TEL` 로 적혀 있지만 사람이 칸을
    더할 때 `Tel` 로 칠 수 있고, 그 한 글자 때문에 보기가 빠지면 아무도 모른다.

    어느 것에도 안 걸리면 배치의 기본값(`Layout.month_choices`)이다. 시트에서
    딸려 온, 팀이 정한 세 모양이 아닌 칸에 남의 보기를 붙이면 그 칸에 적혀
    있던 옛 글을 읽는 사람이 잘못 짚는다.
    """
    haystack = (label or "").upper()
    for needle, choices in layout.month_picks:
        if needle.upper() in haystack:
            return choices
    return layout.month_choices
...
def as_column(row: ContactColumn, layout: Optional[Layout] = None) -> Column:
    ...
    지금까지의 스타트업 리마인드 값이다 — 부르는 곳을 다 고치지 않아도 옛
    동작이 그대로 나오게.

    보기만 칸 이름을 함께 본다(`month_choices`) — 같은 명단 안에서도 문자·
    통화·카톡방이 서로 다른 것을 적는 칸이기 때문이다.
    """
    ...
    return Column(label=row.label, key=note_key(row.id), width=layout.month_width,
                  ...
                  choices=month_choices(layout, row.label))
