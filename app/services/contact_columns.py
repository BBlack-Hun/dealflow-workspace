...
STARTUP_LAYOUT = Layout(
    ...
    head=[
        ...
        Column("계약서 수신여부", "invoice_received", 140, source="note",
               kind="pick", choices="O,X"),
        # ── 달에 매이지 않는 상태 세 칸 (제안서 ①·②·④) ──────────────────
        #
        # **표에 세우고, 월별 칸 바로 앞에 둔다.** 달마다 칸이 세 개씩 붙는
        # 표라 뒤(`tail`)에 두면 달이 쌓일수록 가로로 밀어야 닿는 자리로
        # 물러난다. 이 셋은 기업을 보는 순간 같이 읽는 값이다 — 지금 투자유치를
        # 하는지, 우리와 할 생각인지, 회신이 왔는지에 따라 그 달에 보낼 말이
        # 달라진다.
        #
        # 위 계약 세 칸 **앞이 아니라 뒤**다. 저 셋은 `이메일` 바로 뒤에 나란히
        # 서기로 이미 정해 둔 자리이고(위 주석 · `tests/test_startup_tab.py` 의
        # `계약여부_앞뒤에_견적서와_계약서_칸이_선다`), 사이에 끼면 그 결정이
        # 조용히 풀린다. 순서를 다시 볼 일이라면 ⑦~⑪ 을 함께 정하는 다음
        # 판에서 한 번에 볼 일이지, 이 판에서 지나가며 바꿀 일이 아니다.
        #
        # **왜 붙박이 칸인가**(사용자가 더하는 `ContactColumn` 행이 아니라).
        # ③⑤⑥ 은 달마다 새로 생겨야 하니 행이 맞지만, 이 셋은 **한 기업에 하나**
        # 다. 행으로 두면 ⓐ 명단마다 사람이 손으로 만들어야 하고 — 만들지 않은
        # 명단만 조용히 이 칸이 없다 — ⓑ 월별 칸 자리에 서서 `VISIBLE_MONTHS`
        # 가 1인 표의 한 달치 자리를 먹고 그 달 기록을 접어 버린다(아래
        # `원본NO`·`담당 이력` 주석이 든 것과 같은 사고다).
        #
        # **이주(migration)는 필요 없다.** `source="note"` 라 값은
        # `VcContact.notes` 의 JSON 키로 들어간다 — 옆의 `견적서 첨부여부` ·
        # `계약여부` 와 같은 자리, 같은 방식이다. 표를 고치지 않으므로 새 이주를
        # 만들 이유가 없고, 아직 값이 없는 줄은 빈 칸으로 선다.
        #
        # 폭은 값이 아니라 **필터를 건 뒤의 머리글**에 맞춘다(옆 칸들과 같은
        # 규칙 — `투자유치 상태 (1) ▾` = 126px · `당사 협업 상태 (1) ▾` =
        # 130px · `회신 상태 (1) ▾` = 105px).
        ...
        #
        # 월별 칸(③) 뒤가 아니라 여기 두는 이유: 이 값은 달에 매이지 않는다.
        # 달 칸 사이에 세우면 달이 바뀔 때 지난달 것으로 접히고, 지금 이 기업이
        # 어느 상태인지를 표에서 읽을 수가 없다.
