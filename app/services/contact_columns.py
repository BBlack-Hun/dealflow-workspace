...
#
# 제안서는 이 칸에 `카톡방 참여여부` 를 **합치자**고 한다. 이번에는 합치지
...
STARTUP_LAYOUT = Layout(
    ...
    head=[
        ...
        # 배치에서 카톡방은 달마다 하나씩 생기는 월별 칸이고(`month_seed` 의
        ...
        # 어긋난다. 붙박이 칸으로 새로 빼는 길도 있지만 그것은 `VcContact.
        # kakao_joined` 와 어느 쪽이 최신인지를 사람이 정해야 하는 일이라
        # 이번 판이 아니다(`KAKAO_CHOICES` 주석이 같은 이유로 합치기를 미뤘다).
        # 그래서 카톡방은 ③⑤ 와 함께 월별 묶음에 남는다 — **회신(④) 바로
        # 뒤**라, "회신 왔나 → 그 달에 뭘 했나" 까지는 이어서 읽힌다.
