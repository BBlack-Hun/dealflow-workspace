...
STARTUP_LAYOUT = Layout(
    ...
    head=[
        ...
        # ── 무료 IR 미팅을 **언제 제공하기로 했는가** ──────────────────────
        #
        # 사용자가 자리를 못 박았다 — 「스타트업 메뉴의 이메일 컬럼 오른쪽으로」.
        # 그대로 `이메일` 바로 뒤다.
        #
        # **아래 「일이 진행되는 차례」 묶음은 안 흐트러진다.** 그 차례가 지키는
        # 것은 ①②(지금 어떤 상태인가) → 계약 세 칸(어디까지 진행했는가)의
        # **상대 차례**이고(`tests/test_startup_tab.py` 의 그 검사도 묶음 자체를
        # 본다), 이 칸은 그 묶음 **앞**에 선다. 누구인가(기업명·성함·연락처·
        # 이메일) 뒤에 "그래서 미팅을 언제 주기로 했나" 가 붙는 자리라, 왼쪽에서
        # 오른쪽으로 읽는 흐름도 끊기지 않는다.
        #
        # **월별 묶음도 안 민다.** 달 칸은 `head` 와 `tail` **사이**에 통째로
        # 서므로(`table_columns`), `head` 안 어디에 끼워도 묶음이 쪼개지지
        # 않는다 — 쪼개지는 자리는 `head`/`tail` 사이뿐이고(위 `카톡방(⑥)`
        # 주석), 여기는 그 자리가 아니다. `수정한 날짜` 는 `head` 의 맨 끝
        # 그대로라 월별 묶음 바로 앞에 계속 선다.
        #
        # ## 저장 자리 — `source="note"`, 판(migration)이 없다
        #
        # `VcContact` 에 이 값을 담을 칸이 없고, 만들 것도 아니다. 이 명단에만
        # 있는 값은 `notes` 에 고정 키로 담는 것이 이 배치의 길이다(모듈 첫머리
        # `note` 갈래 · 옆의 `견적서 첨부여부`·`계약여부`가 그렇다). 306행 전체가
        # 한 명단에서만 쓰는 빈 칸을 지고 다니지 않게 한 그것이다.
        #
        # **IR 기업 현황의 `미팅제공일자` 와 다른 칸이다** —
        # 저쪽은 `IrCompany.meeting_offered_at` 이라는 기업 단위의 모델 칸이고,
        # 이쪽은 이 명단의 줄에 `notes` 로 붙는다. 여기에 적어도 저쪽은 안
        # 따라오고 그 반대도 같다. 둘을 하나로 모으는 일은 어느 쪽이 최신인지를
        # 사람이 정해야 하는 일이라 이 판에서 하지 않는다
        # (`models.IrCompany.contract_received` 주석이 같은 것을 셋으로 두고 만
        #  자리를 적어 두었다).
        #
        # ## 갈래 — 날짜 칸이 **따로 없다.** 글자 칸이다
        #
        # `Column.kind` 에는 `text | long | pick | email` 뿐이고 날짜 갈래가
        # 없다. 만들지 않는다 — 이 저장소는 날짜를 적는 칸을 이미 **글자로**
        # 두고 있고, 그 이유가 이 칸에 그대로 걸린다:
        #
        #   · IR 기업 현황의 같은 이름 칸(`companies.html` 의 `미팅제공일자`)이
        #     `<input type="text">` 다. 달력 고르개로 두면 `9월 중`·`미정` 을
        #     적을 수가 없다 — 미팅 날짜는 잡히기 전에 먼저 적히는 값이다.
        #   · 같은 화면의 `수신일` 도 같은 이유로 글자다. 실데이터 281곳 중
        #     14곳이 `날짜 미정` 인데, **수정창은 모든 칸을 한 번에 보내므로**
        #     날짜 칸이면 다른 칸을 고치려고 [저장]하는 것만으로 그 14곳이
        #     지워진다(`companies.html` 의 그 자리 주석). 이 배치의 수정창도
        #     같은 모양이다(`contacts.js` 가 `[data-note]` 를 통째로 싣는다).
        #
        # `pick` 도 아니다. 날짜는 줄마다 달라 고를 거리가 아니고, `pick` 으로
        # 두면 머리글에 필터가 붙어(`Column.filterable`) 줄 수만큼 항목이 생긴다
        # — 옆의 `수정한 날짜` 가 필터를 안 다는 것과 같은 이유다.
        #
        # 적는 꼴은 안내로만 건넨다(`hint` → 수정창 placeholder). 강제하지
        # 않는다 — 강제할 수 있었으면 위 두 칸도 그렇게 했을 것이다.
        #
        # ## 폭 104px — **값이 아니라 머리글**이 정했다
        #
        # 값(`2026-09-15` · 10자)은 13px 본문 글꼴로 71.4px 이고(헤드리스 크롬
        # 실측) td 좌우 여백 18px 을 더해 90px 면 된다. 그런데 머리글
        # `IR 미팅 제공일자` 는 12px 굵은 글씨로 78.5px + 18px = 97px 라,
        # 값에 맞추면 머리글이 두 줄로 접힌다 — 그러면 머리글 줄이 들쭉날쭉해지고
        # 표가 아래로 밀린다. 그래서 넓은 쪽인 머리글에 맞춘다.
        #
        # **저장소의 어림자로 한 번 더 잰다.** `tests/test_startup_tab.py` 의
        # `머리글은_필터_단추까지_한_줄에_들어간다` 는 그려진 화면의 폭과
        # `tests/test_ui_layout.py` 의 `_text_px`(실측으로 맞춘 어림값)를
        # 견주는데, 그 자로는 이 이름이 83.4px + 18px = 101px 이다. 실측보다
        # 4.9px 넉넉한 쪽이라 **그쪽에 맞춘다** — 둘 중 좁은 쪽에 맞추면 검사가
        # 두 줄이라고 짚는다. 여기에 3px 여유를 두어 104px.
        #
        # **`수정한 날짜`(130px)보다 좁다.** 저 칸은 `2026-09-23 14:30` 이
        # 들어가는 폭이고(16자), 이 칸은 날짜만 들어간다 — 같은 날짜 칸이라고
        # 폭까지 맞추면 그 20px 이 값 오른쪽의 빈자리로 남는다. 달마다 세 칸씩
        # 늘어나는 표라 안 쓰는 20px 이 곧 가로 밀림이다.
        #
        # 표 폭(`min-width`)은 **적어 둔 칸 폭의 합**이라
        # (`contacts.html` 의 `table_columns|sum(attribute='width')`) 저절로
        # 따라 늘고, 손으로 맞출 데가 없다.
        Column("IR 미팅 제공일자", "ir_meeting_offered_at", 104,
               source="note", hint="예: 2026-09-15"),
        # ── 여기부터는 **일이 진행되는 차례**다 ────────────────────────────
        #
        ...
        # (`2026-09-16T19:44:43+09:00` → `2026-09-16 19:44`). 라우터가 거기서
        # 한 번 만들어 행에 싣는다(`routers/contacts.contact_rows`) — 표와
        # 응답이 각자 자르면 같은 값이 두 꼴로 보이고 한쪽만 고쳐지는 날이 온다.
        # **초를 뺀 것은 사용자가 나중에 다시 정한 것이다**("모든 수정한
        # 날짜에서 초는 빼줘"). 이 화면은 그 한 곳을 지나기만 하므로 고칠 것이
        # 폭밖에 없었다 — 왜 빼는지와 오프셋을 떼는 이유는 그 함수에 적혀 있다.
        #
        # ## 고칠 수 없다
        #
        ...
        # 가 안 쓰이는 필터 속성으로 잡는다.
        #
        # ## 폭 130px — **이름이 아니라 값**에 맞춘다
        #
        # 값이 **언제나 16자**(`2026-09-16 19:44`)라, 13px 본문 글꼴로 110px
        # 이다(헤드리스 크롬 실측 108.1px). 여기에 td 좌우 여백 18px 을 더하면
        # 128px — 2px 여유를 두어 130px. 머리글(`수정한 날짜` = 56px + 18)보다
        # 값이 훨씬 넓은 칸이라 이 표의 다른 칸들(필터 건 뒤의 머리글에 맞춘다)과
        # 자가 다르다. IR 기업 현황의 같은 칸도 같은 이유로 130px 이다(#200).
        #
        # **150 → 130px.** 초를 빼면서(`clock.stamp_text`) 글자가 19자에서
        # 16자로 줄어든 만큼 다시 쟀다 — 안 줄이면 그 20px 이 값 오른쪽의
        # 빈자리로 남는다. 표 폭은 칸 폭의 합이라(`contacts.html`) 저절로
        # 20px 따라 줄고, 손으로 맞출 데가 없다.
        #
        # 한 줄로 고정한다(`app.css` 의 `.startup-table td.updated-at`) —
        # 달 칸이 늘어 표가 넓어지는 날 빈칸 자리에서 접히면 그 줄만 키가 큰다.
        Column("수정한 날짜", "updated_at", 130, source="stamp"),
    ],
    tail=[
