...
class Layout:
    ...
    # 완료` 와 `연결 완료` 가 한 목록에 섞여, 무엇을 고른 것인지 세지 못한다.
    #
...
# ── 스타트업 화면의 **골라 넣는 칸들** ──────────────────────────────────────
#
# 여섯이었다가 **다섯이 되었다** — ④(`회신 상태`)는 표에서 뺐다. 값도 보기
# 목록(`REPLY_CHOICES`)도 그대로 두었다. 왜 뺐는지와 되돌리는 법은 아래
...
#
# 기존 문자 칸에 적혀 있던 `연결` 은 이 보기에 없다. 보냈다는 기록이 아니라
# **보낸 뒤에 돌아온 반응**이라, ④(`회신 상태`)가 적던 말이다. 그 칸은 표에서
# 뺐으므로(아래 `REPLY_CHOICES` 주석) 이제 그런 말은 **메모**로 간다.
SEND_CHOICES = "미발송,발송 예정,발송 완료,발송 실패,발송 제외"
# ④ 문자·카톡 등 연락 이후의 반응. **이 보기를 쓰던 `회신 상태` 칸은 표에서
# 뺐다** — 왜 뺐는지와 되돌리는 법은 아래 `STARTUP_LAYOUT.head` 의 그 자리
# 주석에 적혀 있다.
#
...
#
# 아래 `CALL_CHOICES` 와 값이 안 겹치는지도 계속 잰다
# (`tests/test_startup_status_choices.py`) — 남아 있는 옛 값과 통화 결과가
...
# 답을 들었다는 것은 다른 말이다. 뒤엣것을 적는 칸은 ②(`당사 협업 상태`)다 —
# ④(`회신 상태`)도 적던 것인데 그 칸은 표에서 뺐다(위 `REPLY_CHOICES` 주석).
# 한 칸으로 합치면 "통화는 됐는데 거절" 을 적을 자리가 없다.
