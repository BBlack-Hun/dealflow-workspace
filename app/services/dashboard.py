...
#
# 사용자가 `100` 뒤로 100단위를 더 달라고 해서 200·300 을 붙였다. **300 에서
# 끊는다** — 이 목록의 모집단은 내 명단(`sheet_owner.my_contacts`)이라 아무리
# 늘려도 그 인원을 못 넘고, DB 전체 투자사 담당자가 334명이다(2026-09 개발 DB).
# 400 을 세워 봐야 300 과 같은 목록이 나오는 단추가 된다.
#
# **지금 실제로 걸리는 인원은 21명이다**(개발 DB 전체를 모집단으로 둬도 21명 —
# IR 자료를 달라고 한 곳이 그만큼뿐이다. 운영에서는 10명). 그래서 100 을
# 눌러도, 300 을 눌러도 목록은 같다. 그 사실을 화면이 말해야 "왜 안 늘지" 가
# 안 된다 — 머리글의 `N명` 칸(`req-rank-count`)과 그 아래 안내 줄
# (`req-rank-all`)이 그 일을 한다. 둘 다 건드리지 마라.
TOP_CHOICES = [10, 30, 50, 100, 200, 300]
TOP_DEFAULT = 50


def clamp_top(top: int) -> int:
    """주소로 들어온 값을 고를 수 있는 범위 안으로 접는다.

    위끝은 `max(TOP_CHOICES)` 를 읽는다 — 목록에 300 을 더하면 여기도 저절로
    300 까지 늘어난다. 숫자를 따로 박아 두면 단추는 300 인데 `?top=300` 이
    100 으로 잘리는 일이 생긴다.
    """
...
#: 달별 반응을 몇 달치 보일지 — 이 달 + 지난 11달.
#:
#: **한 번 둘로 줄였다가 사용자가 넷으로 되돌렸고, 이번에 사용자가 열둘로
#: 정했다.** 그러니 "둘이면 충분하다" 는 판단은 이미 한 번 틀린 것이다 —
#: 달 수를 다시 줄이려거든 사용자에게 먼저 물어라.
#:
#: 자료는 여전히 넉 달치뿐이다(시트에서 옮겨 온 활동이 6월부터, 이 앱에서
#: 쌓인 요청·미팅이 8월부터). **열둘로 늘리면 앞선 여덟 달은 전부 0 이다** —
#: 2026-09 에 개발 DB 로 세어 본 값이다. 그래도 사용자가 열둘을 원한 까닭은
#: "언제부터 쌓였는지" 를 화면에서 보려는 것이라, 0 인 달을 감추지 않는다.
#: 0 은 흐리게만 둔다(`.month-react .zero`).
#:
#: 예전 주석은 "더 늘리면 390px 폰에서 칸이 서로 밀려 숫자가 줄바꿈된다" 고
#: 경고했다. **그 경고는 표가 `행 = 반응 · 열 = 달` 이던 때의 것이다.** 열둘로
#: 늘리며 표를 뒤집었다(`행 = 달 · 열 = 반응 4가지`) — 이제 칸 수는 달 수와
#: 무관하게 늘 다섯이라, 달을 더 늘려도 가로로는 넓어지지 않고 세로로만
#: 길어진다. 왜 뒤집었는지는 `templates/dashboard.html` 의 그 패널 주석에 잰
#: 값과 함께 적어 두었다.
REACTION_MONTHS = 12
...
def admin_dashboard(db: Session, today: Optional[date] = None) -> dict:
    ...
    return {
        ...
        # 미팅 후기 **발송 대기 목록 자동 준비**. 메일·문자와 나란히 선다 —
        # 서버가 때맞춰 알아서 하는 것들이 한 화면에 모인다. 켜짐/꺼짐만이
