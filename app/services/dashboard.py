...
#: 달별 반응을 몇 달치 보일지 — **이 달 + 지난 달**, 둘이다.
#:
#: 처음에는 넷이었다(자료가 6월부터 4달치라서). 사용자가 둘로 줄여 달라고
#: 정했다 — 이 패널에서 보는 것은 "이번 달이 지난달보다 나은가" 하나이고,
#: 넉 달을 세우면 그 견줌이 넷 중 하나로 묻힌다.
#:
#: 줄이는 쪽은 안전하다. 늘리는 쪽만 조심하면 된다 — 자료가 없는 달은 0 만
#: 늘어서고, 390px 폰에서 칸이 서로 밀려 숫자가 줄바꿈된다.
REACTION_MONTHS = 2
