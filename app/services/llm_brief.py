...
def groups(db: Session, user: User) -> List[dict]:
    """자료에 담긴 투자사의 **그룹 갈래와 인원** — `[{"name", "count"}, …]`.
    ...
    **모집단도 자료와 같아야 한다** — `investor_rows` 한 곳이 정한다. 화면
    ...
    그 갈래를 찾을 수 없다.
    """
    rows = sheet_owner.group_rows(investor_rows(db, user))
    return [{"name": g["label"], "count": g["count"]} for g in rows]
...
def prompt() -> str:
    ...
    **그룹마다 따로 고르라고 시키지 않는다.** 그것은 이 함수가 예전에 하던
    "투자사마다 8곳" 으로 되돌아가는 일이다(위 첫 문단). 한 벌은 하나로 두고,
    그 한 벌 안에서 어느 것이 어느 그룹에 맞는지만 덧붙이게 한다.
    ...
    **뜻을 지어내지 말라고 못 박는다.** 그룹 이름은 뜻이 보이는 것
    ...
    한 명단 실측(투자사 114곳 · 기업 321곳)에서 자료는 약 72,700자이고 그중
    투자사 쪽이 13,300자(18%)다. 그룹 한 칸을 줄마다 더하면 1,600자가 늘어
    **자료 전체의 +2.3%** 다. 무거운 쪽은 기업 목록(82%)이고, 그룹 때문에
    투자사를 잘라 내면 **잘린 사람은 추천을 못 받는다** — 그 손해가 2.3%
    보다 훨씬 크다.

    갈래 표(`groups`)를 따로 싣는 것도 같은 셈이다. 갈래 수만큼이라 수백 자를
    넘지 않는데, 없으면 읽는 쪽이 114줄을 세어야 하고 **세다 틀린 비례는
    겉보기에 멀쩡하다.**
    ...
    return "\n".join([
        ...
        "그룹 이름으로 말씀해 주시면 앱에서 그대로 보낼 수 있습니다. 그룹마다 "
        "**그 갈래에 속한 분들이 적어 둔 말을 모아** 어떤 곳인지 한 줄로 적어 "
        "주세요 — 이름에 뜻이 있으면(`Pre IPO` · `Series C 이상` · `Seed`) 그 "
        "이름이 곧 조건이고, `A` · `B` 처럼 글자뿐이면 **뜻을 지어내지 마시고** "
        "속한 분들이 적어 둔 말로만 보세요. "
        f"`{GROUPS_KEY}` 에 갈래가 하나뿐이면 이 단계는 건너뛰세요.",
        "2. 투자사들이 적어 둔 말 — 관심 분야(`sectors`) · 투자 단계(`stages`) · "
        ...
        "**그 세 칸이 비어 있어도 그룹이 적혀 있으면 1번에서 읽어낸 그 그룹의 "
        "성격으로 세어 주세요** — 그 분에 대해 아는 것이 그룹뿐인 경우가 많습니다.",
        f"3. 센 수요의 **비례대로 {PICK_COUNT}자리를 분야에 나눠 주세요.** "
        "몇 명이 원해서 몇 자리를 줬는지 함께 적어 주세요 — 나눈 근거가 보여야 "
        "사람이 고쳐 잡을 수 있습니다.",
        "4. 자리마다 기업을 고를 때는 그 수요가 말한 **투자 단계·투자 규모**에 "
        "맞는 곳으로 골라 주세요.",
        "5. 계약(유료·무료)으로 이어질 만한 자리는 **`revenue_over` 가 `"
        ...
        "그런지만 밝혀 주세요.",
        "6. 많은 투자사가 `sent_before` 에 이미 담고 있는 기업은 **빼 주세요** — "
        "한 벌을 전원에게 보내므로, 이미 받아 본 사람이 많은 곳은 새로 소개할 "
        "것이 못 됩니다.",
        f"7. 고르고 나니 {PICK_COUNT}곳이 안 되면 되는 만큼만 적고 몇 곳인지 "
        "밝혀 주세요. 수를 채우려고 안 맞는 곳을 넣지 마세요.",
        # **한 벌은 그대로 하나다.** 그룹마다 따로 고르라는 뜻이 아니다 —
        # 고른 한 벌 안에서 어느 것이 어느 그룹에 맞는지만 덧붙인다. 딜 소개를
        # 그룹 단위로 보내므로(`deal_queue.targets`) 이 줄이 있어야 사람이
        # 답을 그대로 발송으로 옮길 수 있다.
        f"8. 고른 {PICK_COUNT}곳은 **한 벌 그대로**입니다. 그 위에, 그룹마다 "
        "**그 한 벌 중 어느 곳이 특히 맞는지**만 덧붙여 주세요(그룹 이름은 "
        f"`{GROUPS_KEY}` 에 적힌 글자 그대로). 예) `Pre IPO: C-7, C-12` — "
        "그룹별로 새로 고르라는 뜻이 아닙니다.",
        f"9. 답은 **번호로** 적어 주세요. 예) {ANSWER_EXAMPLE}",
        "",
