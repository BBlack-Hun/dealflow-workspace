...
TABLE_LABELS = {
    ...
    "ref_sheets": "참고 자료",
    # ★ 이 줄이 **되돌리기에서 제일 조심할 것**이다. 자동 발송은 이 표에 남긴
    # 표시로 "이미 보냈다" 를 안다(`models.AutoSendRun`). 되돌려서 표시가 줄면
    # 그만큼이 **다시 나간다** — 받는 쪽은 투자사다. 그래서 이름을 달아 둔다:
    # 되돌리기 화면의 표에서 이 줄이 음수면, 누르기 전에 그 뜻을 알아야 한다.
    "auto_send_runs": "자동 발송 표시(두 번 안 보내게 하는 것)",
    "auto_send_settings": "자동 발송 설정",
}
