...
TABLE_LABELS = {
    ...
    "ref_sheets": "참고 자료",
    # ★ 이 줄이 **되돌리기에서 제일 조심할 것**이다. 결과 문의 대기 목록은 이
    # 표에 남긴 표시로 "이미 세웠다" 를 안다(`models.AutoSendRun`). 되돌려서
    # 표시가 줄면 그만큼이 **다시 목록에 선다** — 받는 쪽은 투자사다. 그래서
    # 이름을 달아 둔다: 되돌리기 화면에서 이 줄이 음수면 그 뜻을 알아야 한다.
    "auto_send_runs": "발송 준비 표시(두 번 안 세우게 하는 것)",
    "auto_send_settings": "발송 자동 준비 설정",
}
