...
function makeEl(tag) {
  ...
  const el = {
    ...
    focus() {}, select() {},
    // 화면을 굴려 그 자리를 보여 주는 것. 이 DOM 에는 그림이 없어 할 일이
    // 없지만 **있어야 한다** — 없으면 폼을 여는 코드(`ir_numbers.js` 의
    // [미팅 잡기])가 검사에서만 죽어, 정작 봐야 할 것(담당자가 제대로
    // 들어갔는가)을 아무도 못 본다.
    scrollIntoView() {},
    // 칸을 벗어나는 것을 **정말로 알린다.** 표에서 칸을 고치는 코드는 저장을
    // `blur` 에 걸어 두는데(`inline_edit.js`), 흉내만 내고 아무에게도 안 알리면
    // 키를 눌러 저장하는 길이 여기서만 조용히 아무 일도 안 한다.
    // **위로 올리지 않는다** — 브라우저의 `blur` 는 버블링하지 않는다.
    blur() {
      var ev = { target: el, stopPropagation() {}, preventDefault() {} };
      (el.handlers.blur || []).forEach(function (fn) { fn(ev); });
    },
