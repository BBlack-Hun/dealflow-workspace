...
def test_칸이_일의_흐름대로_선다(sheets):
    ...
    덩어리로 서기 때문이다(`contact_columns.table_columns`). 대신 회신 상태
    **바로 뒤**에 와서 "회신 왔나 → 그 달에 뭘 했나" 까지는 이어서 읽힌다 —
    아래가 그것까지 함께 본다.
    """
    head = _thead(sheets.get(_url(LIST)).text)
    flow = ["기업명", "투자유치 상태", "당사 협업 상태",
            "견적서 첨부여부", "계약여부", "계약서 수신여부", "회신 상태"]
    at = [head.index(name) for name in flow]
    assert at == sorted(at), (
        "칸 차례가 일의 흐름과 다릅니다:\n"
        f"  흐름 {flow}\n  화면 {head}")
    # 달에 매이는 칸(문자·통화·카톡방)은 전부 그 뒤다.
    assert head.index("회신 상태") < head.index(MONTHS[0]), head
    # 회신 바로 뒤가 월별 묶음이어야 둘이 이어서 읽힌다.
    assert head[head.index("회신 상태") + 1] == MONTHS[0], head
