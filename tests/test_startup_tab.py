...
#
# 차례는 **일이 진행되는 순서**다. 담당 기업이 여든인 팀원이 적어 보낸 말이
# 그것을 정했다 — "각 월의 문자/전화여부가 먼저 나오는 게 아니라, 각 기업이
# 투자유치 진행하시는지 / 우리와 카톡연결 되어있는지 / 견적서 전달했는지
# 계약서 전달했는지 / 그래서 계약하셨는지 이 순서로 흐름이 읽혀야".
#
#   누구인가 → 미팅을 언제 주기로 했나 → 투자유치를 하는가 → 우리와 하는가
...
HEAD = ["NO", "기업명", "성함", "연락처", "이메일",
        # 사용자가 자리를 못 박았다 — 「이메일 컬럼 오른쪽」
        # (`tests/test_startup_ir_meeting_date.py`). 아래 흐름 묶음 **앞**이라
        # ①②→계약 세 칸의 상대 차례는 그대로다.
        "IR 미팅 제공일자",
        "투자유치 상태", "당사 협업 상태",
...
def test_계약여부_앞뒤에_견적서와_계약서_칸이_선다(sheets):
    ...
    **묶음이 통째로 어디 서는지는 여기서 안 정한다.** 한동안 `이메일` 바로
    뒤로 못 박아 두었는데, 그러면 그 앞에 아무 칸도 못 세운다 — 사용자가 읽고
    싶다고 한 차례(투자유치 → 협업 → 견적서 → 계약)를 세우려면 ①② 가 이 셋
    앞에 서야 하고, 그 자리를 이 검사가 막고 있었다. 이 검사가 지키는 것은
    **셋의 상대 차례**이고, 묶음의 자리는 아래 `흐름` 검사가 통째로 본다.
    """
...
def test_칸이_일의_흐름대로_선다(sheets):
    """표를 왼쪽에서 오른쪽으로 읽으면 **그 기업이 어디까지 왔는지**가 나온다.

    담당 기업이 여든인 팀원이 이렇게 적어 보냈다 — "각 월의 문자/전화여부가
    먼저 나오는 게 아니라, 각 기업이 투자유치 진행하시는지 / 그래서 우리와
    카톡연결 되어있는지 / 견적서 전달했는지 계약서 전달했는지 / 그래서
    계약하셨는지 이 순서로 흐름이 읽혀야 하는 거 아닌가요?"

    칸을 하나 더 넣는 일은 앞으로도 있을 텐데, **넣는 사람이 폭이나 갈래를
    보고 자리를 고르면 이 차례가 조용히 흐트러진다** — 화면은 멀쩡하고 읽는
    사람만 다시 헤맨다. 그래서 차례 자체를 여기서 못 박는다.

    카톡방(⑥)은 사용자가 말한 자리(협업 상태와 견적서 사이)에 못 선다. 이
    배치에서 카톡방은 **달마다 하나씩 생기는 월별 칸**이고 월별 칸은 한
    덩어리로 서기 때문이다(`contact_columns.table_columns`). 대신 계약 세 칸
    **바로 뒤**에 와서 "어디까지 진행했나 → 그 달에 뭘 했나" 까지는 이어서
    읽힌다 — 아래가 그것까지 함께 본다.

    한동안 이 자리에 `회신 상태` 가 있었다(계약서 수신여부와 월별 묶음 사이).
    사용자 요청으로 뺐고, 그 뒤로는 계약 세 칸이 월별 묶음과 이웃한다.
    """
    head = _thead(sheets.get(_url(LIST)).text)
    flow = ["기업명", "투자유치 상태", "당사 협업 상태",
            "견적서 첨부여부", "계약여부", "계약서 수신여부"]
    at = [head.index(name) for name in flow]
    assert at == sorted(at), (
        "칸 차례가 일의 흐름과 다릅니다:\n"
        f"  흐름 {flow}\n  화면 {head}")
...
def test_투자사_관리_현황의_필터는_한_개도_안_바뀐다(sheets):
    ...
    **재는 것은 필터 키다.** 머리글 이름은 바뀔 수 있고 실제로 바뀌었다
    (`담당자` → `연결 담당` · `관심도 (월말기준)` → `관심도`). 바뀌면 안 되는
    것은 키 쪽이다 — 대시보드 링크와 저장해 둔 주소가 그 키로 들어온다
    (`tests/test_edit_panel_labels.py` 가 이름과 키를 따로 지킨다).
    """
