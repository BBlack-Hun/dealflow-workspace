...
def test_계약까지_가는_세_칸이_흐름_안에서_나란히_선다(lists):
    """월별 칸 뒤에 두면 달이 쌓일수록 표 끝으로 밀린다 — 가로로 밀어야 닿는다.

    셋은 **일이 일어나는 순서대로 붙어 서야 한다**(견적서 → 계약 → 계약서).
    떼어 놓으면 한 기업이 어디까지 갔는지를 표 세 군데서 모아야 한다.

    **`이메일` 바로 뒤로 못 박지 않는다.** 한동안 그랬는데, 그러면 그 앞에
    아무 칸도 못 세운다 — 사용자가 읽고 싶다고 한 차례(투자유치 → 협업 →
    견적서 → 계약)를 세우려면 상태 두 칸이 이 셋 앞에 서야 한다. 차례 전체는
    `tests/test_startup_tab.py` 의 `칸이_일의_흐름대로_선다` 가 본다.
    """
