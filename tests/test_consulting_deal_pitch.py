...
def test_스타트업_탭에서만_서는_칸들_끝에_선다(allowed, db, users):
    ...
    처음에는 `기업 관리` 바로 오른쪽이었다. 지금은 그 사이에 견적서 첨부 여부 →
    계약완료여부 → 계약서 수신완료여부 세 마디가 들어와 있고
    (`tests/test_consulting_contract_done.py`), 소개 문구는 그 뒤로 물러났다 —
    세 마디는 일이 나아가는 차례라 `기업 관리` 에 붙어 있어야 읽히고, 소개
    문구는 그 흐름과 다른 축의 메모다.
...
def test_표_모양은_이름이_아니라_열쇠로_짝짓는다(db):
    ...
    assert consulting.STARTUP_COLUMNS[-2] == ("딜 소개문구", "deal_pitch")
