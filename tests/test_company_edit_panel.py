...
[수정] 창에 아예 없었다.
    ...
    수신일 · 대표자 · 연락처 · 이메일
    22·23·24·25년 매출 · 설립년도 · 정책자금(그때 이름 `기보, 신보, 중진공`)
    사업 설명(business_desc)
...
def test_창의_이름은_표_머리글과_한_글자도_다르지_않다():
    """`정책자금` 을 창에서 `보증기관` 이라 부르면 같은 칸인 줄 모른다.

    위 `STARTUP_DB_FIELDS` 의 이름 둘이 2026-09 에 바뀌었다(사용자 요청):
    `대표자` → `대표자명` · `기보, 신보, 중진공` → `정책자금`. 표와 창을
    **함께** 바꿔야 이 검사가 통과한다 — 그게 이 검사가 하는 일이다.
...
def test_재료_칸의_이름은_한_곳에서만_정해진다():
    ...
    `사업분야` 로는 못 돌아간다: 옆 탭의 `대분류`(`sector_major`)와
    ...
    assert name != "사업분야", "옆 탭의 `대분류`(예전 이름 `사업분야 대분류`) 와 같은 말입니다"
