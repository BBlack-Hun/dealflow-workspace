...
def test_the_contract_sheet_reads_month_and_kind_from_the_line(tmp_path):
    ...
    # 슬래시 줄은 나누기 전 한 줄 그대로 넘어간다 — 나누는 것은 앱의 규칙이다
    assert all(g["name"] == "" for g in got)


# --- 이름을 쉼표로 묶은 줄 ---------------------------------------------------
#
# 같은 탭 아래쪽(`누적`)에서는 이름을 쉼표로 묶어 **한 줄**에 적는다. 슬래시가
# 없다고 건너뛰었더니 이 줄들이 통째로 빠졌다. 여기서 지키는 것은 넷이다 —
# 기업 하나에 한 줄인가 · 무료/유료가 맞게 붙는가 · **슬래시 모양이 여전히
# 되는가** · 시트에 없는 값(계약금액·성공보수율·계약일)을 지어내지 않는가.


def _contract(rows):
    """`월간 계약 업무현황표` 모양의 가상 시트 → (줄 목록, 사람에게 할 말)."""
    import sys
    ...
    for row in rows:
        ws.append(row)
    notes = []
    return parse_contract_sheet(ws, notes), notes


def test_쉼표로_묶은_줄은_기업_수만큼_줄이_된다():
    got, notes = _contract([
        ["누적", "", ""],
        ["무료 계약 기업", "샘플가, 샘플나, 샘플다", "3개"],
        ["유료 계약 기업", "샘플라, 샘플마", "2개"],
    ])
    # 무료/유료는 **왼쪽 라벨**에서 읽는다 — 이 모양은 줄 안에 그 말이 없다.
    assert [(g["name"], g["kind"]) for g in got] == [
        ("샘플가", "무료"), ("샘플나", "무료"), ("샘플다", "무료"),
        ("샘플라", "유료"), ("샘플마", "유료")]
    # 원본 줄은 그대로 남는다(한 묶음에서 나온 줄은 같은 줄을 가리킨다)
    assert got[0]["line"] == "샘플가, 샘플나, 샘플다"
    assert not notes


def test_누적_아래의_줄은_달을_지어내지_않는다():
    """바로 위 달을 물려받으면 그 달의 계약이 아닌 것이 그 달로 들어간다.

    시트가 `누적` 이라고 적어 두었으니 적힌 그대로 적는다.
    """
    got, _ = _contract([
        ["6월  (무료계약 1개사)", "", ""],
        ["무료 계약", "샘플가/ 무료/ 3.5%/ 미정", ""],
    ...
    assert [(g["month"], g["name"]) for g in got] == [
        ("6월", ""), ("누적", "샘플나"), ("누적", "샘플다")]


def test_두_모양이_섞여_있어도_둘_다_읽는다():
    """슬래시 모양이 원래 쓰던 것이다 — 쉼표 줄을 읽느라 깨뜨리면 안 된다."""
    got, _ = _contract([
        ["6월  (무료계약 1개사 / 유료계약 1개사)", "", ""],
        ["무료 계약", "기업명 / 계약금액 / 성공보수율 / 계약일", ""],
        ["유료 계약", "샘플가/ 무료/ 3.5%/ 미정", ""],
        ["", "샘플나/ 유료 90만/ 4%/ 8", ""],
        ["누적", "", ""],
    ...
    assert [(g["month"], g["kind"], g["name"], g["line"]) for g in got] == [
        ("6월", "무료", "", "샘플가/ 무료/ 3.5%/ 미정"),
        ("6월", "유료", "", "샘플나/ 유료 90만/ 4%/ 8"),
        ("누적", "무료", "샘플다", "샘플다, 샘플라"),
        ("누적", "무료", "샘플라", "샘플다, 샘플라"),
    ]


def test_머리글_줄은_쉼표로_적혀_있어도_값이_아니다():
    got, _ = _contract([
        ["누적", "", ""],
        ["", "기업명, 계약금액, 성공보수율", ""],
        ["무료 계약 기업", "샘플가, 샘플나", "2개"],
    ])
    assert [g["name"] for g in got] == ["샘플가", "샘플나"]


def test_빈_줄은_줄을_만들지_않는다():
    assert _contract([])[0] == []
    assert _contract([
        ["", "", ""],
        ["누적", "", ""],
        ["무료 계약 기업", "", ""],       # 라벨만 있고 이름이 없다
        ["", "", ""],
    ])[0] == []


def test_개수_칸과_안_맞으면_넣되_말한다():
    """개수 칸이 이 줄의 유일한 검산이다. 조용히 맞추지 않는 것은 개수와
    이름 중 어느 쪽이 맞는지 여기서 알 수 없어서다."""
    got, notes = _contract([
        ["누적", "", ""],
        ["유료 계약 기업", "샘플가, 샘플나", "3개"],
    ])
    assert [g["name"] for g in got] == ["샘플가", "샘플나"]   # 넣기는 넣는다
    assert len(notes) == 1
    assert "3개" in notes[0] and "2개" in notes[0]


def test_상호_안의_쉼표는_그냥_나누고_알린다():
    """`샘플가, 샘플나 주식회사` 가 한 회사인지 두 회사인지는 글자만 보고
    가릴 수 없다. 나누되 개수 칸과 어긋난 것을 말해 준다."""
    got, notes = _contract([
        ["누적", "", ""],
        ["유료 계약 기업", "샘플가, 샘플나 주식회사", "1개"],
    ])
    assert [g["name"] for g in got] == ["샘플가", "샘플나 주식회사"]
    assert notes and "쉼표" in notes[0]
