...
(그 칸은 2026-09 에 **`투자라운드` 로 이름이 바뀌었다** — 같은 칸이고
`ir_companies.series` 그대로다. 요청을 옮겨 적은 윗줄은 그때 말 그대로 둔다.)
그 한 줄이 실제로 지켜지려면 서로 다른 네 가지가 동시에 참이어야 한다.

1. **자리** — `투자라운드` 바로 뒤다. 자리는 요청의 절반이라 여기서 못 박는다
   (칸을 하나 더 끼워 넣다가 순서가 밀리면 요청과 달라진다).
2. **꼴** — 날짜와 **시·분**이 보인다. 저장값은 `2026-09-16T16:31:40+09:00` 라
   ...
   **초는 뺀다 — 처음에는 남겼었다.** 첫 요청이 '시분초까지' 여서 19자를
   ...
   날짜에서 초는 빼줘"). 그래서 이 아래 검사들은 **초가 없는 것**을 지킨다 —
   `[:10]` 으로 더 잘라 버리는 것도 여전히 막는다.
...
def test_투자라운드_바로_뒤에_선다():
    """요청의 절반은 **자리**다 — `투자라운드` 다음 칸이어야 한다.

    이름이 `기업구분` 이던 칸이다(2026-09 에 바뀌었다 — 같은 `series` 칸).
    여기서 재는 것은 **화면에 서는 머리글**이라 이름을 같이 바꿔야 한다.
    """
    ...
    assert "투자라운드" in names, f"`투자라운드` 머리글이 없습니다: {names}"
    assert names.index(COLUMN) == names.index("투자라운드") + 1, (
        f"`{COLUMN}` 은 `투자라운드` **바로 뒤**여야 합니다 — 지금 차례: {names}")
...
def test_시분까지_보인다():
    """`2026-09-16T16:31:40+09:00` → `2026-09-16 16:31`."""
    from app.clock import now_iso, stamp_text

    assert stamp_text("2026-09-16T16:31:40+09:00") == "2026-09-16 16:31"
    # 지금 적히는 값도 같은 자리에서 끊긴다 — 저장 꼴이 바뀌면 여기서 잡힌다.
    text = stamp_text(now_iso())
    assert re.fullmatch(r"\d{4}-\d{2}-\d{2} \d{2}:\d{2}", text), text


def test_초를_자른다():
    """사용자가 다시 정했다 — "모든 수정한 날짜에서 초는 빼줘".

    **한 곳에서만 자른다.** 이 검사가 지키는 것은 꼴 하나가 아니라 자르는
    자리가 하나라는 사실이다 — 화면이나 라우터에서 또 자르기 시작하면 두 화면이
    서로 다른 꼴을 보이게 되고, 그중 하나만 고쳐지는 날이 온다.

    날짜까지만(`[:10]`) 잘라 버리는 것은 여전히 안 된다. 이 저장소에는
    `sent_at[:10]` 관용구가 스무 곳 넘게 있어 그것을 옮겨 쓰기 쉽다.
    """
    from app.clock import stamp_text

    text = stamp_text("2026-09-16T16:31:40+09:00")
    assert text.endswith(":31"), text
    assert ":40" not in text, f"초가 남아 있습니다: {text}"
    assert len(text) == 16, f"날짜까지 잘라 버렸습니다: {text}"


def test_마이크로초가_붙은_옛_값도_같은_자리에서_끊긴다():
    """`now_iso` 는 초까지 적지만, 그 전에 적힌 값까지 책임진다."""
    from app.clock import stamp_text

    assert stamp_text("2026-08-01T09:00:00.123456+00:00") == "2026-08-01 09:00"
...
def test_폭은_값에_맞춘다():
    ...
    **150 → 130px.** 초를 빼면서 값이 19자에서 16자가 됐다. 폭은 값이 정하므로
    값이 줄면 폭도 같이 줄어야 한다 — 안 줄이면 20px 이 빈자리로 남는다.

    헤드리스 크롬으로 실측(13px 본문 글꼴): `2026-09-16 19:44` 이 108.1px
    (전에는 `…:43` 까지 127.5px 였다). td 좌우 여백 18px 을 더하면 126.1px 이라
    값 쪽이 128px 을 요구한다. 머리글은 정렬 단추 `수정한 날짜 ↕` 68.5px + 18px
    = 87px 이라 값보다 좁다.

    `%` 가 아니라 `px` 인 이유는 companies.html 의 그 칸 주석에 적어 두었다 —
    값이 16자로 고정이라 표가 넓어져도 더 보여 줄 것이 없고, `%` 로 두면
    ...
    assert int(m.group(1)) >= 128, f"값 110px + td 여백 18px 이 안 들어갑니다: {m.group(1)}px"
    # 넉넉하다고 그냥 두면 안 된다 — 빈자리도 폭이다.
    assert int(m.group(1)) < 148, (
        f"초를 뺐는데 폭이 19자 시절 그대로입니다: {m.group(1)}px")
...
def test_표_min_width_이_칸_폭을_따라간다():
    """든 만큼만 올리고, **준 만큼만 내린다.**

    올릴 때 안 올리면 폭을 안 준 `딜 소개 문구` 가 새 칸 폭을 통째로 내어
    준다 — 이 표에서 제일 많이 읽는 칸이라 깎을 자리가 아니다. 반대로 칸이
    좁아졌는데 표 폭을 그대로 두면, 줄어든 20px 을 `%` 칸들이 비율대로 나눠
    가져간다 — 초를 뺐는데 기업명 칸이 넓어지는 엉뚱한 일이 된다.
    """
    css = CSS.read_text(encoding="utf-8")
    m = re.search(r"#co-table \{[^}]*min-width:\s*(\d+)px", css)
    assert m, "`#co-table` 의 min-width 를 못 찾았습니다"
    assert int(m.group(1)) == 2386 + WIDTH_PX, (
        f"`{COLUMN}` 은 {WIDTH_PX}px 인데 표 폭이 {m.group(1)}px 입니다 — "
        f"{2386 + WIDTH_PX}px 여야 합니다")
