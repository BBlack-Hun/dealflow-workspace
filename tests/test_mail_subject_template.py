...
def test_테스트_표시는_치환_뒤에_그대로_붙는다(logged, db, seed, mail_on, monkeypatch):
    """★ 테스트 모드에서 나가는 메일임을 제목이 계속 말해야 한다."""
    from app import config

    monkeypatch.setattr(config, "TEST_ROOM", "테스트방")
    job = _send(logged, seed, subject="{투자사} 딜 소개")
    assert _subject(db, job) == "[테스트] 가나벤처스 딜 소개"
