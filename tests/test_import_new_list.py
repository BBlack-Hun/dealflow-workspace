...
def test_원본번호와_담당_이력은_달마다_늘어나는_칸이_아니다(
    ...
    걸리면 두 가지가 한꺼번에 나빠진다.
      ...
      · 시트에서 이 둘이 그 달의 기록보다 **앞에** 적혀 있다. 달을 못 읽는
        칸은 혼자 한 묶음이라(`contact_columns.split_months`) 펴 둘 달 자리를
        하나씩 먹는다 — 정작 그 달의 기록이 접히고 표에는 원본 번호가 선다.
    ...
    visible, _folded = cc.split_months(months, today=date(2026, 8, 15))
    assert [c.label for c in visible][:3] == seeded, \
        f"이번 달 세 칸이 표 맨 앞에 안 섰습니다: {[c.label for c in visible]}"
