...
def test_send_targets_are_connected_only(logged, db):
    ...
    **`화면에 글자가 있나` 로 보지 않는다.** 연결 전 사람도 이제 화면에는 뜬다 —
    목록에서만 빼 두었더니 누가 왜 빠졌는지 알 길이 없어, 카톡방을 아무리
    확인해도 안 움직이는 수를 계속 들여다보게 됐다(딜 제안 관리의 `빠진 사람`
    칸). 갈리는 것은 **고를 수 있는가**다: 발송이 사람을 집어 가는 표식
    (`data-name`)이 붙어 있으면 체크가 걸리고 그대로 나간다.
    """
    _import(logged, [
        _row(1, "연결됨", kakao="O"),
        _row(2, "진행중", kakao="X", memo="신규연결 진행", firm="마바벤처스"),
    ...
    assert 'data-name="연결됨"' in body
    assert 'data-name="진행중"' not in body, "연결 전인데 고를 수 있다"
    # 그래도 **보이기는 해야 한다** — 안 보이면 왜 빠졌는지 알 수가 없다.
    assert "진행중" in body, "빠진 사람이 화면에서 통째로 사라졌다"
