...
이 표만의 판단들을 못 박는다.
  ...
  · 세울 값은 **화면 글자가 아니다.** `소개 횟수` 는 `6회 · 180명` 으로 그려지고
...
# **이름 둘이 바뀌었다**(2026-09, 사용자 요청): `사업분야 대분류` → `대분류` ·
# `기업구분` → `투자라운드`. 여기 적힌 것은 **화면에 실제로 서는 머리글**이라
# 같이 바꿔야 한다 — 안 바꾸면 이 검사가 없는 칸을 찾으며 통과해 버려,
# 정렬 단추가 새 이름의 칸에 붙어도 아무도 못 잡는다.
# (바꾼 자리 전부는 tests/test_company_column_renames.py 에 적혀 있다.)
UNSORTED_COLUMNS = ["NO", "기업명", "대분류", "소분류", "투자라운드",
...
def test_글자_칸에는_안_붙인다(column):
    ...
    꼬리표(` ▲`)까지 더하면 `대분류 (1) ▾ ▲` 가 되어 두 줄로 접힌다 —
...
def test_소개_횟수는_회차_수로_센다(logged_in, db, users):
    """화면은 `6회 · 180명` 이지만 세우는 것은 **앞의 수**다.

    칸 이름이 곧 그 수이고 화면에서도 굵게 서 있는 쪽이다. `건수` 로 세우면
    이름과 세우는 값이 달라져, `소개 횟수` 내림차순에서 `3회 · 200명` 이
    `9회 · 180명` 보다 위에 선다.
