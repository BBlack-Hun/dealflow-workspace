...
| 리스크 | 내용 | 완화책 |
|---|---|---|
| **카카오 운영정책 위반 소지** | 카카오톡 운영정책은 자동화 프로그램(매크로 등)을 통한 서비스 이용을 금지 → **계정 이용 제한(제재) 가능성**. 본 도구의 최대 리스크 | 사람 유사 패턴(건당 랜덤 3~7초 지연), 1회 발송 상한 60건, **원칙적으로 수동 트리거**(예외는 §1.6), 사용자 감독 모드 기본, 사용자에게 최초 실행 시 리스크 고지·동의 화면 |
...
### 1.6 무인 자동 발송 — **미팅 후기 한 종류만** (사용자 결정으로 원칙 완화)

§1.5 는 오래 **"수동 트리거만(무인 스케줄 발송 금지)"** 이었다. **사용자가 그
리스크를 알고, 미팅 후기 한 종류에 한해 무인 발송을 하기로 정했다.** 문서와
코드가 갈리면 둘 다 못 믿으므로 여기에 적어 둔다 — 구현은
`app/services/auto_send.py`, 설정 화면은 팀 현황(`/team`)이다.

**무엇이 바뀌었나**: 방아쇠를 당기는 주체 하나뿐이다. 건당 랜덤 지연(3~7초),
1회 발송 상한 60건, 반자동 폴백(§8)은 **그대로 지킨다.**

**적용 범위**: 미팅 후기(미팅 완료 +10일, 그 미팅을 한 투자사 담당자에게)
**하나뿐이다.** 딜소개·IR 자료 전달·후속 캐던스는 지금도 사람이 [발송 시작] 을
누른다.

**위험을 좁히는 장치**(전부 코드로 강제한다 — 화면 안내가 아니다)

| 장치 | 값 | 왜 |
|---|---|---|
| 기본값 | **꺼짐** | 아무것도 안 넣으면 아무 일도 일어나지 않는다(메일·문자와 같다) |
| 보내는 계정 | **한 계정**(설정에서 고른다) | 팀 전체가 아니라 정해진 한 사람의 PC 에서만 나간다 |
| 보내는 때 | 평일, 설정한 시각의 창. **창은 09~19시 안에서만** 고를 수 있다 | 받는 쪽이 투자사다 — 새벽·주말 발송은 그 자체로 사고 |
| 하루 상한 | 기본 10건 · **최대 60건** | §1.5 의 `1회 상한 60건` 을 넘지 않는다 |
| 중복 방지 | 건마다 `auto_send_runs` 에 **보내기 전** 표시, `(kind, ref_id)` 유일 | 결과 문의는 담당자가 결과를 적기 전까지 목록에 남아 있어, 막지 않으면 같은 투자사에게 **매일** 간다 |
| 끄기 | 팀 현황에서 `꺼짐` 고르고 [저장] — 그다음 회차부터 잡을 만들지 않는다 | 멈추는 길이 분명해야 한다 |
| 사후 확인 | 평범한 발송 잡(`send_jobs`)이라 `/jobs/{id}` 와 `최근 발송 회차` 에 남는다(회차 제목에 `(자동)`) | 자동이라 아무도 안 보면 잘못 나간 것을 영영 모른다 |
| 시험방 | `DEALFLOW_TEST_ROOM` 이 있으면 **거기로만** 나간다 | 수동 발송과 같은 길을 지나므로 저절로 지켜진다 |

**"그 PC" 로 묶을 수 있는가 — 지금은 계정까지다.** 잡은 그 계정의 기기 토큰으로만
내려가고(`agent_devices.user_id` 가 UNIQUE 라 **계정당 기기 하나**), 그래서 계정으로
막는 것과 PC 로 막는 것이 지금 구조에서는 같은 일이다. 다만 토큰은 설정 파일에
담겨 내려가므로 **파일을 복사하면 다른 PC 에서도 돈다** — `agent_devices.hostname`
은 발송기가 스스로 말한 값을 적기만 하고 대조하지 않는다. 진짜 PC 묶기가 필요하면
(ㄱ) 기기 등록 시 지문(hostname+MAC 등)을 굳혀 두고 (ㄴ) 폴링마다 대조해 다르면
거절하고 (ㄷ) 기기 교체를 관리자가 승인하는 절차가 있어야 한다. 이번에는 만들지
않았다.
...
| 미팅 후기 무인 발송(§1.6)이 잘못 나감 | 팀 현황에서 **끄면 즉시 멈춘다**(다음 회차부터 잡을 만들지 않는다). 이미 선 잡은 진행 화면에서 [중단]. 나간 것은 `/jobs` 와 `최근 발송 회차`(제목에 `(자동)`)에서 확인 |
