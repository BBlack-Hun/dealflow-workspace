...
| 무엇 | 어디에 사나 | 누가 정하나 |
|---|---|---|
| 쓸 수 있는 계정 | `users.can_auto_attach_ir` | **관리자**가 팀 현황에서 (0059) |
...
**누가 자동으로 붙이나 — 관리자가 켜 준 계정이, 폴더를 정해 두었을 때**

두 길이 함께 산다. 위 두 칸이 **둘 다** 찬 계정은 **발송기가 파일을 먼저 보내고
문구를 뒤에** 보내고, 그 밖에는 지금까지처럼 **문구만 나가고 사람이 PC 카톡에서
손으로 붙인다**(발송 화면에 안내창이 뜬다). 판정은 한 곳이다
(`app/services/ir_attach.py: auto_attach_enabled`).

계정을 이름으로 박지 않았다 — 사람이 바뀌면 낡는다. 대신 **계정마다 켜고 끄는
칸 하나**를 둔다. `users.can_view_consulting`(투자현황) 과 같은 자리, 같은
모양이다: 관리자가 팀 현황에서 켜고 끄고, 역할은 새 계정의 기본값만 정한다
(`deps.auto_attach_default_for` — **지금은 어느 역할도 켜지 않는다**).

> 0059 이전에는 문이 `ir_root` 하나였다. 그런데 그 칸은 **본인이 넣는 값**이라
> 문이 곧 스위치였고, 누구든 스스로 켤 수 있었다. 지금 `ir_root` 는 "어느
> 폴더인가" 라는 제 뜻만 지고, "쓸 수 있는가" 는 관리자가 정한다.
>
> **꺼도 넣어 둔 폴더는 지우지 않는다** — 읽지 않을 뿐이라(`/setup` 의 칸도,
> 발송기에 내려보내는 박동도, 발송 목록도 전부 `deps.may_auto_attach` 를 읽는다)
> 다시 켜면 그대로 돌아온다.
>
> 이미 폴더를 넣어 두고 쓰던 계정은 **이주(0059)가 켜 둔 채로 옮겼다** — 배포한
> 순간 아무도 기능을 잃지 않는다.

폴더 칸을 여전히 함께 보는 이유는 **없으면 어차피 못 보낸다**는 것이다: 폴더를
모르면 발송기는 파일을 한 개도 찾을 수 없다. 켜 주기만 하고 폴더가 없는 계정을
켜진 것으로 치면 자료 없이 문구만 나간다.
...
**내가 안 한 변경**만 모아 두는 자리다. 곧 팀원이 서로의 명단을 고칠 수 있게
되는데, 이 명단은 곧 발송 대상이라 남이 바꾼 것을 아무도 모르면 그대로
오발송이 된다.

| | 남는가 |
|---|---|
| 남의 담당자 명단·컨설팅 줄·계정을 고쳤다 | 남는다 (`남의 것`) |
| 주인 없는 공용 자료를 고쳤다 (IR 기업 · 딜 소싱 · 딜 제안 문구 · 참고 자료 · 월 열 · 팀 설정) | 남는다 (`공용`) |
| **자기 것만 고쳤다** | **안 남는다** |
| 화면을 열기만 했다(GET) | 안 남는다 |
| 발송·백업 같은 배경 작업이 만든 줄 | 안 남는다 |

- **한 곳을 지난다.** 라우터마다 로그 호출을 붙이지 않는다 — SQLAlchemy 세션
  이벤트 한 곳에서 ORM 으로 들어가는 모든 저장을 잡는다
  (`app/services/edit_log.py` 머리말에 후보 셋을 견준 근거가 있다). 새 화면이
  생겨도 저절로 남고, 빠지면 `tests/test_edit_log.py` 가 라우트와 표를 통째로
  훑어 잡는다.
- **값은 허용 목록에 있는 칸만 남는다.** 담당·명단·단계·카톡방 이름·켜짐꺼짐
  처럼 잘못되면 발송이 어긋나는 칸이다. 메모·연락처·긴 글은 `바뀜` 표시만
  남고, **비밀번호·토큰은 어떤 경우에도 값이 남지 않는다.**
- **관리자만 본다.** 보관 정책은 13개월(오래된 것을 지우는 일은 아직 만들지
  않았다 — 지금은 쌓이기만 한다).
