...
**미팅 후기만 무인으로 나간다.** 미팅이 끝나고 열흘 뒤, 사람이 아무것도 누르지
않아도 그 투자사에게 후기 카톡이 나가는 길이 하나 있다. **기본은 꺼짐**이고,
관리자가 팀 현황(`/team`)에서 보내는 계정·시각·하루 상한을 정해 켠다. 왜 문서의
"수동 트리거만" 원칙을 완화했는지와 안전선은 `docs/TECH_SPEC.md` §1.6 ·
`app/services/auto_send.py` 에 적어 두었다 — **되돌리기 전에 그것부터 읽을 것.**
나머지(딜소개·IR 자료 전달·후속 캐던스)는 지금도 사람이 [발송 시작] 을 누른다.
